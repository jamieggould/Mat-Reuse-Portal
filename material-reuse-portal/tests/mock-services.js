...
http.createServer((req, res) => {
  let b = ''; req.on('data', (c) => (b += c)); req.on('end', () => {
    ...
    if (p === '/v3/smtp/email') { log.push({ brevoMail: JSON.parse(b) }); res.end('{"messageId":"m1"}'); return; }
    if (p === '/v3/transactionalSMS/sms') { log.push({ brevoSms: JSON.parse(b) }); res.end('{"reference":"s1"}'); return; }
