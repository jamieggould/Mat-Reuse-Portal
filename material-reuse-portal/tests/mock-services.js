...
http.createServer((req, res) => {
  let b = ''; req.on('data', (c) => (b += c)); req.on('end', () => {
    ...
    if (p === '/v1/messages') { const j = JSON.parse(b); log.push({ claude: j.messages.slice(-1)[0].content, sys: j.system.length }); const last = j.messages.slice(-1)[0].content; res.end(JSON.stringify({ content: [{ type: 'text', text: /refund/i.test(last) ? 'I’ve passed this to the team — they’ll email you. [ESCALATE: refund requested for ORD-2026-1300]' : 'Mock answer: your order is awaiting collection.' }] })); return; }
