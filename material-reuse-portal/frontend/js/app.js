...
const PAGES = [
  ...
  { id: 'warehouse', label: 'Online Warehouse', ico: '📦' },
  { id: 'orders', label: 'Orders & Collections', ico: '🚚' },
  { id: 'lists', label: 'Shopping Lists', ico: '📋' },
  { id: 'carbon', label: 'Carbon Reporting', ico: '🌱', gate: (g) => g.carbonReports },
  { id: 'projects', label: 'Projects & Audits', ico: '🏗️', gate: (g) => g.projects },
  { id: 'membership', label: 'Membership & Billing', ico: '💳' },
  { id: 'settings', label: 'Account Settings', ico: '⚙️' },
];
...
function renderNav() {
  ...
  $('#nav').innerHTML = PAGES.map((p) => {
    ...
    return `<button data-page="${p.id}" class="${locked ? 'locked' : ''} ${state.page === p.id ? 'active' : ''}"
      onclick="go('${p.id}')"><span class="ico">${p.ico}</span>${p.label}</button>`;
  }).join('');
}
...
async function go(page) {
  ...
  try { await RENDER[page](); } catch (e) { $('#view').innerHTML = `<div class="empty">⚠️ ${esc(e.message)}</div>`; }
  window.scrollTo(0, 0);
}
...
RENDER.dashboard = async () => {
  ...
  $('#view').innerHTML = `
    <div class="tagline-strip">
      ...
      <button class="btn btn-sm" style="background:var(--navy);color:#fff" onclick="go('warehouse')">Browse the warehouse →</button>
    </div>
    ...
    <div class="grid cols-4" style="margin-top:18px">
      ...
      <div class="card stat"><span class="stripe" style="background:var(--orange)"></span>
        ...
        <div class="sub">${g.shoppingListLimit === null ? 'Unlimited on your plan' : `${lst.lists.length} of ${g.shoppingListLimit} on Domestic Free`}</div></div>
    </div>

    <h3 class="section-title">🆕 Just landed in the warehouse ${inv.earlyAccessVisible ? '<span class="pill pill-green">48h early access active</span>' : ''}</h3>
    ...
    <div class="grid cols-2" style="margin-top:28px">
      <div class="card">
        ...
        ${active.length ? active.slice(0, 3).map((o) => `
          <div class="doc-row"><span class="ico">${o.type === 'donation' ? '♻️' : '📦'}</span>
      ...
      <div class="card">
        <h3 style="margin-bottom:12px">${g.projects ? '🏗️ Your projects' : '🌱 Why membership matters'}</h3>
        ${g.projects
          ? (prj.projects.length ? prj.projects.slice(0, 3).map((p) => `
              <div class="doc-row"><span class="ico">🏗️</span>
...
function itemCard(i) {
  return `<div class="item-card">
    <div class="item-thumb">${i.emoji}
      ${i.earlyAccess ? '<span class="early pill pill-navy">⚡ Early access</span>' : ''}
      <span class="qty">${i.quantity.toLocaleString('en-GB')} ${esc(i.unit)}</span>
    </div>
    <div class="item-body">
      <h4>${esc(i.title)}</h4>
      <div class="item-meta">${esc(i.category)} · ${esc(i.condition.split('—')[0].trim())}</div>
      <div class="item-carbon">🌱 saves ${i.carbonSavedKgPerUnit} kg CO₂e / ${esc(i.unit.replace(/s$/, ''))}</div>
      <div class="item-foot">
  ...
  const draw = async () => {
    ...
    if (!$('#fCat')) {
      $('#view').innerHTML = `
        ...
        ${earlyAccessVisible ? '<p class="small" style="margin-bottom:14px"><span class="pill pill-navy">⚡</span> <b>Early access on</b> — you see new stock 48 hours before Domestic Free members.</p>'
          : '<p class="small muted" style="margin-bottom:14px">🔒 Some new stock is in 48-hour early access for Domestic Plus members and above.</p>'}
...
async function openPassport(sku) {
  ...
  const g = state.tier.gates;
  const canReserve = g.reservations && i.quantity > 0;
  $('#modalRoot').innerHTML = `
  <div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal">
      <button class="close" onclick="closeModal()">✕</button>
      <span class="pill ${i.passportVerified ? 'pill-green' : 'pill-yellow'}">${i.passportVerified ? '✓ Product passport verified' : 'Passport being finalised'}</span>
      ...
      <div class="passport-grid">
        <div class="f"><b>Quantity</b><span>${i.quantity.toLocaleString('en-GB')} ${esc(i.unit)}${i.areaCoverage ? ' · ' + esc(i.areaCoverage) : ''}</span></div>
        <div class="f"><b>Condition</b><span>${esc(i.condition)}</span></div>
        <div class="f"><b>Sent from</b><span>${esc(i.sentFrom)}</span></div>
        <div class="f"><b>Delivered by</b><span>${esc(i.deliveredBy)}</span></div>
        <div class="f"><b>Date added</b><span>${fmtDate(i.dateAdded)}</span></div>
        <div class="f"><b>Available from</b><span>${fmtDate(i.availableFrom)}</span></div>
        <div class="f"><b>Lifecycle stage</b><span>${esc(i.lifecycleStage)}</span></div>
        <div class="f"><b>Price</b><span>${fmtGBP(i.price)} ${esc(i.priceUnit)}</span></div>
        <div class="f"><b>Embodied carbon (new)</b><span>${i.carbonNewKgPerUnit} kg CO₂e / ${esc(i.unit.replace(/s$/, ''))}</span></div>
        <div class="f"><b>Carbon saved by reuse</b><span style="color:#1d7a05;font-weight:700">${i.carbonSavedKgPerUnit} kg CO₂e / ${esc(i.unit.replace(/s$/, ''))}</span></div>
      </div>
      <p class="small muted">🌱 ${esc(i.carbonNote)}</p>
      <p class="small" style="margin:10px 0">${esc(i.notes)}</p>
      ...
      <div style="display:flex;gap:10px;margin-top:18px;flex-wrap:wrap">
        <button class="btn btn-primary" ${canReserve ? '' : 'disabled'} onclick="reserveItem('${i.sku}')">
          ${g.reservations ? 'Reserve for collection' : '🔒 Reserving needs Domestic Plus+'}</button>
...
RENDER.orders = async () => {
  ...
  const orderBlock = (o) => `
    <div class="card order-card">
      <div class="order-head">
        <div><h4>${o.type === 'donation' ? '♻️' : '📦'} ${esc(o.id)}</h4>
      ...
      <div class="order-foot">
        <span>🗓️ <b>${esc(o.slot)}</b></span>
        ${o.total ? `<span>💷 ${fmtGBP(o.total)}${o.deliveryFee ? ` <span class="muted small">(+${fmtGBP(o.deliveryFee)} delivery)</span>` : ''}</span>` : ''}
        ${o.memberDiscount ? `<span class="pill pill-green">member discount −${fmtGBP(o.memberDiscount)}</span>` : ''}
        <span>🌱 ${fmtKg(o.carbonSavedKg)} CO₂e saved</span>
      </div>
  ...
  $('#view').innerHTML = `
    ...
    ${dons.length ? `<h3 class="section-title">♻️ Donation lots (materials you’ve sent us)</h3>${dons.map(orderBlock).join('')}` : ''}
    ...
    ${lists.length ? lists.map((l) => {
      ...
      return `<div class="card" style="margin-bottom:16px">
        <div class="order-head">
          ...
          <div style="text-align:right"><b style="font-family:var(--font-head)">${fmtGBP(total)}</b><br><span class="small" style="color:#1d7a05">🌱 ${fmtKg(carbon)} CO₂e</span></div>
        </div>
        ${l.items.length ? `<table><tr><th>Item</th><th>Qty</th><th>Est. cost</th><th>Stock</th><th></th></tr>
          ${l.items.map((i) => `<tr>
            <td>${i.emoji || '📦'} ${esc(i.title || i.sku)}</td>
            <td>${i.qty}</td>
...
RENDER.carbon = async () => {
  const g = state.tier.gates;
  if (!g.carbonReports) {
    $('#view').innerHTML = upgradeNote('Carbon reporting', 'Domestic Plus');
    return;
  }
  ...
  $('#view').innerHTML = `
    ...
    ${r.wlcaModules ? `<div class="card" style="margin-top:18px">
      <h3 style="margin-bottom:10px">🏛️ WLCA module breakdown (kg CO₂e)</h3>
    ...
    <div style="margin-top:18px;display:flex;gap:10px;flex-wrap:wrap">
      ${g.carbonReports === 'full' || g.carbonReports === 'verified'
        ? `<button class="btn btn-primary" onclick="toast('Report queued — a PDF will land in your inbox shortly')">⬇️ Download ${g.carbonReports === 'verified' ? 'verified' : ''} carbon report (PDF)</button>`
        : `<button class="btn btn-ghost" disabled>⬇️ Downloadable reports — Community tier and above</button>`}
    </div>`;
};
...
RENDER.projects = async () => {
  const g = state.tier.gates;
  if (!g.projects) {
    $('#view').innerHTML = upgradeNote('Projects & audits', 'Community');
    return;
  }
  const { projects } = await api('/api/projects?userId=' + state.user.id);
  $('#view').innerHTML = `
    ${g.audits ? `<div class="tagline-strip" style="margin-bottom:18px">
      <span>🏛️ Need a GLA-compliant pre-demolition audit? Your partner rate includes Lawmens strip-out at 5% off.</span>
      <button class="btn btn-sm" style="background:var(--navy);color:#fff" onclick="toast('Request sent — your account manager will be in touch within one working day')">Request an audit</button>
    </div>` : ''}
    ${projects.length ? projects.map((p) => `
      <div class="card proj-card">
        <div class="order-head">
          <div><h4>🏗️ ${esc(p.name)}</h4>
        ...
        ${p.documents.length ? `<h4 style="font-size:13px;margin:14px 0 4px">Documents</h4>
          ${p.documents.map((d) => `<div class="doc-row"><span class="ico">📄</span>
    ...
    <div class="tier-grid">
      ${tiers.map((t) => `
        <div class="tier-card ${t.id === u.tier ? 'current' : ''}">
          ...
          <div class="price">${t.price === 0 ? 'Free' : fmtGBP(t.price).replace('.00', '')} <span>${esc(t.billing)}</span></div>
          <ul>${t.features.map((f) => `<li>${esc(f)}</li>`).join('')}</ul>
          ${t.id === u.tier
            ? '<button class="btn btn-ghost" disabled>Your plan</button>'
            : `<button class="btn ${t.id === 'corporate-reuse-partner' ? 'btn-primary' : 'btn-green'}" onclick="switchTier('${t.id}','${esc(t.name)}')">
                ${tiers.findIndex((x) => x.id === t.id) > tiers.findIndex((x) => x.id === u.tier) ? 'Upgrade' : 'Switch'} to ${esc(t.name)}</button>`}
        </div>`).join('')}
    </div>
  ...
  $('#view').innerHTML = `
    <div class="grid cols-2">
      ...
      <div>
        ...
        <div class="card" style="margin-top:16px">
          <h3 style="margin-bottom:8px">ℹ️ Membership</h3>
