...
const statusPill = (s) => {
  const map = {
    ...
    'Logged — emission assessment': 'pill-orange', 'Paid': 'pill-green', 'Overdue': 'pill-red', 'Cancelled': 'pill-red',
  ...
  $('#view').innerHTML = `
    ...
    <div class="grid cols-4" style="margin-top:18px">
      ...
      <div class="card stat"><span class="stripe" style="background:var(--orange)"></span>
        <div class="lbl">Materials offered</div><div class="big">${openReq.length}</div>
        <div class="sub">${openReq[0] ? esc(openReq[0].status) : 'No open requests'}</div></div>
    </div>
    ...
    <div class="grid cols-2" style="margin-top:28px">
      ...
      <div class="card">
        <h3 style="margin-bottom:12px">Materials you’ve offered</h3>
        ${rq.requests.length ? rq.requests.slice(0, 3).map((r) => `
              <div class="doc-row">
                <span class="t">${esc(r.id)} — ${esc(r.projectName || 'New project')}<span>${r.materials.length} line${r.materials.length === 1 ? '' : 's'} · ${esc(r.location)}</span></span>
                ${statusPill(r.status)}</div>`).join('')
          : `<p class="small muted" style="margin-bottom:14px">Strip-out, refit or clearance coming up? Offer the materials and we’ll arrange recovery, passports and rehoming.</p>
             <button class="btn btn-primary btn-sm" onclick="go('offer')">Offer materials</button>`}
      </div>
    </div>
