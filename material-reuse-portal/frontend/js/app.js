...
function toggleRegister(ev) {
  ...
  $('#loginToggleNote').innerHTML = showingLogin
    ...
    : 'New to Material Reuse Group? <a href="#" onclick="toggleRegister(event)">Create a free account</a> — Domestic Free Membership, upgrade any time.';
}
...
async function doRegister(ev) {
  ...
  try {
    ...
    $('#regPassword').value = '';
    enterApp(user, tier);
    toast('Welcome to Material Reuse Group — your free membership is live');
  } catch (e) {
    err.textContent = e.message; err.classList.add('show');
  }
...
RENDER.membership = async () => {
  const u = state.user;
  const { membership: m, tiers } = await api('/api/membership');
  const features = [...new Set(tiers.flatMap((t) => t.features))];
  const cur = tiers.find((t) => t.id === u.tier) || tiers[0];
  ...
  const action = (t) => {
    ...
    if (t.id === 'community') return m.communityRequest ? '<span class="pill pill-yellow">Request pending</span>' : `<button class="btn btn-ghost btn-sm" onclick="changeMembership('community')">Apply</button>`;
  ...
  $('#view').innerHTML = `
    ...
    ${m.communityRequest ? `<div class="tagline-strip" style="margin-bottom:18px"><span>Your Community Free Membership request for <b>${esc(m.communityRequest.organisation)}</b> is with the team — you’ll get an email when it’s reviewed.</span></div>` : ''}
    ...
    <div class="plans">
      <table class="plan-table">
        <thead><tr><th class="feat-col"><div class="plan-intro">Compare<br>features</div></th>${tiers.map((t) => `<th class="${t.id === u.tier ? 'current' : ''}"><div class="plan-name">${esc(t.name)}</div><div class="plan-price">${t.price ? fmtGBP(t.price) : '£0'}<span> / ${esc(t.billing.toLowerCase())}</span></div><div class="plan-tag">${esc(t.tagline || '')}</div></th>`).join('')}</tr></thead>
        <tbody>${features.map((f) => `<tr><td class="feat-col">${esc(f)}</td>${tiers.map((t) => `<td class="${t.id === u.tier ? 'current' : ''}">${t.features.includes(f) ? '<i class="tick">✓</i>' : '<i class="dash">—</i>'}</td>`).join('')}</tr>`).join('')}
        <tr class="actions"><td class="feat-col"></td>${tiers.map((t) => `<td class="${t.id === u.tier ? 'current' : ''}">${action(t)}</td>`).join('')}</tr></tbody>
      </table>
    </div>
    <p class="small muted" style="margin-top:10px">Paid plans are billed annually by card through Stripe and renew automatically. Upgrades take effect immediately (you pay the difference for the rest of the year); downgrades and cancellations take effect at the end of the paid year. Community Free Membership is for community groups and is approved by the MRG team.</p>
    ...
    <div class="grid cols-2">
      <div class="card">
        ...
        <p class="small" style="margin-top:6px"><b>Method:</b> ${esc(u.billing.method || (cur.price ? 'Card (Stripe)' : 'No payment needed'))}<br><b>${m.pendingTier === 'domestic-free' ? 'Ends' : 'Renews'}:</b> ${m.renews ? fmtDate(m.renews) : '—'}</p>
...
async function changeMembership(tierId) {
  ...
  if (tierId === 'community') { const org = prompt('Community Free Membership is for community groups. What is the name of your group or organisation?', state.user.organisation || ''); if (org === null) return; body.organisation = org.trim(); }
  else if (tierId !== state.user.tier && !confirm(tierId === 'domestic-free' ? 'Switch to Domestic Free Membership? Paid plans end at the end of the paid year.' : 'Change your membership plan?')) return;
  try {
