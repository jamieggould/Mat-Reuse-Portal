...
RENDER.dashboard = async () => {
  ...
  if (!g.passports) { // domestic tiers: just what matters to them
    ...
    <div class="grid cols-3">
      <div class="card cta-card"><h3>Browse the Reuse Hub</h3><p class="small">Reclaimed materials with a passport — buy outright or reserve with a deposit; it lands straight on your account.</p>
        <button class="btn btn-green btn-sm" onclick="MATF.scope='stock';go('materials')">Open the Reuse Hub</button></div>
    ...
    <div class="grid cols-2" style="margin-top:18px">
      ...
      <div class="card">
        ...
        ${ord.orders.length ? ord.orders.slice(0, 4).map((o) => `<div class="doc-row"><span class="t">${esc(o.id)} — ${(o.items || []).map((l) => `${l.qty} × ${esc(l.title || l.sku)}`).join(', ')}<span>${fmtDate(o.placed)}${o.total ? ' · ' + fmtGBP(o.total) : ''}</span></span>${statusPill(o.status)}</div>`).join('')
          : `<p class="small muted" style="margin-bottom:14px">Nothing yet — buy or reserve from the Reuse Hub and it will show here with its collection slot.</p><button class="btn btn-primary btn-sm" onclick="MATF.scope='stock';go('materials')">Open the Reuse Hub</button>`}
      </div>
    </div>`;
    return;
  }
...
RENDER.orders = async () => {
  ...
  $('#view').innerHTML = `
    ${buys.length ? buys.map(orderBlock).join('') : '<div class="card"><div class="empty">No orders yet — buy or reserve from the Reuse Hub.</div></div>'}
