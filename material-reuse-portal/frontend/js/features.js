const perItemKg = (x) => (Number(x.quantity) > 1 ? (Number(x.carbonSavedKg) || 0) / Number(x.quantity) : (Number(x.carbonSavedKg) || 0));
/* =====================================================================
...
(function wireNav() {
  ...
  after(PAGES, 'warehouse', [
    { id: 'materials', label: 'Marketplace' },
  ]);
  // Not in this release: the Softr warehouse embed (the marketplace now lives on the passports page) and Offer Materials.
  ['lists', 'warehouse', 'offer'].forEach((id) => { const i = PAGES.findIndex((p) => p.id === id); if (i >= 0) PAGES.splice(i, 1); });
  { const pj = PAGES.find((p) => p.id === 'projects'); if (pj) pj.label = 'Projects'; }
  ['warehouse'].forEach((id) => { const i = ADMIN_PAGES.findIndex((p) => p.id === id); if (i >= 0) ADMIN_PAGES.splice(i, 1); });
  Object.assign(HIDDEN_PAGES, { offer: { id: 'offer', label: 'Offer materials' }, warehouse: { id: 'warehouse', label: 'Online warehouse' }, adminRequests: { id: 'adminRequests', label: 'Material requests' } });
  after(PAGES, 'orders', [
    { id: 'documents', label: 'Documents', gate: (g) => g.documents },
    { id: 'impact', label: 'Impact & ESG', gate: (g) => g.impactCentre },
  ]);
  after(ADMIN_PAGES, 'adminMembers', [
    { id: 'adminMaterials', label: 'Marketplace & Passports' },
  ]);
...
function showLoginForm(ev) {
  ...
  $('#loginToggleNote').innerHTML = 'New to Material Reuse Group? <a href="#" onclick="toggleRegister(event)">Create a free account</a> — Domestic Basic is free, upgrade any time.';
...
async function doReset(ev) {
  ...
  try {
    const r = await api('/api/auth/reset', { method: 'POST', body: { token: RESET_TOKEN, password: $('#rsPassword').value } });
    history.replaceState(null, '', '/');
    if (r.signIn) { showLogin(); toast('Password updated — sign in with your new password and authenticator code'); return; }
    setToken(r.token); enterApp(r.user, r.tier); toast('Password updated — you’re signed in');
  } catch (e) { err.textContent = e.message; err.classList.add('show'); }
  ...
  <div class="public-wrap"><div class="public-card">
    ...
    <div class="public-body">
      ...
      <div class="passport-grid" style="margin-top:14px">
        ...
        <div class="f"><b>Carbon saved by reuse</b><span style="color:#1d7a05;font-weight:700">${fmtKg(perItemKg(x))} CO₂e${Number(x.quantity) > 1 ? ` each · ${fmtKg(x.carbonSavedKg || 0)} for all ${x.quantity}` : ''}${x.carbonEstimated ? ' (est.)' : ''}</span></div>
        ${x.weightKg ? `<div class="f"><b>Weight diverted</b><span>${fmtT(x.weightKg)}${x.weightEstimated ? ' (est.)' : ''}</span></div>` : ''}
        ${x.listedPrice ? `<div class="f"><b>Listed price</b><span>${fmtGBP(x.listedPrice)} ${esc(x.priceUnit || '')} · ${esc(x.availability || '')}${x.retailNew > x.listedPrice ? `<br><span class="small muted">would cost ≈ ${fmtGBP(x.retailNew)} new</span>` : ''}</span></div>` : ''}
      </div>
      ${(x.history || []).length ? `<h4 style="margin:18px 0 8px;font-size:12px;letter-spacing:.1em;text-transform:uppercase;color:#5E6B85">Provenance</h4>${historyHTML(x.history, true)}` : ''}
...
(function loggedOutDeepLinks() { // runs on load, before /me resolves
  ...
  else window.__pendingPassport = m[2];
})();

/* provenance chain — every passport carries its own history */
const historyHTML = (h, compact = false) => (h && h.length) ? `<div class="timeline ${compact ? 'compact' : ''}">${h.map((e) => `
  <div class="tl"><i></i><div><b>${esc(e.event)}</b><span class="small muted"> · ${fmtDate(e.date)}</span>${e.detail ? `<div class="small">${esc(e.detail)}</div>` : ''}</div></div>`).join('')}</div>` : '';
...
async function uploadFile(file, userId, priv = false) {
  if (file.size > 10 * 1024 * 1024) throw new Error(`${file.name} is over 10 MB`);
  const data = await readAsDataURL(file);
  const { file: f } = await api('/api/uploads', { method: 'POST', body: { name: file.name, type: file.type, data, userId, private: priv } });
  return f;
}
...
RENDER.materials = async () => {
  ...
  const simple = !isAdmin && !state.tier.gates.passports; // domestic tiers: the marketplace is the shop
  if (simple) MATF.scope = 'stock';
  if (!isAdmin) { try { CREDIT = (await api('/api/membership')).membership; } catch (e) { CREDIT = null; } }
  ...
  $('#view').innerHTML = `
    ${CREDIT && CREDIT.evidenceOutstanding && CREDIT.evidenceOutstanding.length ? `<div class="tagline-strip" style="margin-bottom:12px;background:#B42318"><span>Buying is paused until you upload installation photos for ${CREDIT.evidenceOutstanding.map(esc).join(', ')}.</span><button class="btn btn-green btn-sm" onclick="go('orders')">Upload photos</button></div>` : ''}
    <div class="tagline-strip">
      <span>${simple ? `Reclaimed materials, each with a passport — buy outright or reserve with a ${META && META.depositPercent ? META.depositPercent : 50}% deposit.${CREDIT && CREDIT.credit && CREDIT.credit.allowance ? ` <b>${fmtGBP(CREDIT.credit.remaining)} credit</b> left this year — applied automatically.` : ''}` : 'Every recovered material carries a permanent digital passport — provenance, condition, carbon and where it went next.'}</span>
      ${isAdmin ? `<button class="btn btn-green btn-sm" onclick="materialModal()">New passport</button>` : `<button class="btn btn-green btn-sm" onclick="document.getElementById('wishlistCard').scrollIntoView({behavior:'smooth'})">Can’t see it? Join the wishlist</button>`}
    </div>
    <div class="filters" style="margin-top:18px">
      ...
      ${!isAdmin && !simple ? `<select id="mfScope" onchange="matFilter()">${optList(['all', 'mine', 'stock'], MATF.scope).replace('>all<', '>All passports<').replace('>mine<', '>My materials<').replace('>stock<', '>MRG warehouse stock<')}</select>` : ''}
      <button class="btn btn-primary btn-sm" onclick="matFilter()">Search</button>
    </div>
    <p class="small muted" style="margin:10px 0 14px">${total} ${simple ? 'item' : 'passport'}${total === 1 ? '' : 's'}</p>
    ${materials.length ? `<div class="mp-grid">${materials.map(mpCard).join('')}</div>`
      : `<div class="card"><div class="empty">No passports match — try clearing a filter.</div></div>`}
    ${!isAdmin ? wishlistCard() : ''}`;
  if (!isAdmin) loadWishes();
};
function wishlistCard() {
  return `<div class="card" id="wishlistCard" style="margin-top:28px">
      <h3 style="margin-bottom:6px">Can’t see what you need? Join the wishlist</h3>
      <p class="small muted" style="margin-bottom:14px">Tell us what you’re after. The team sees every request, and you’re emailed${state.user.phone ? ' and texted' : ''} the moment a matching item is listed.</p>
      ${state.tier.gates.materialSourcing ? `
      <div class="form-grid" style="grid-template-columns:1fr 1fr 110px">
        <div><label>Category</label><input id="wlCat" list="wlCats" placeholder="e.g. Seating"><datalist id="wlCats">${(META ? META.categories : []).map((c) => `<option value="${esc(c)}">`).join('')}</datalist></div>
        <div><label>Item / keywords</label><input id="wlType" placeholder="e.g. task chairs, oak doors"></div>
        <div><label>How many</label><input id="wlQty" type="number" min="1" placeholder="Optional"></div>
        <div style="grid-column:1/-1"><label>Anything else — sizes, condition, when you need it</label><input id="wlNotes" placeholder="Optional"></div>
      </div>
      <div class="modal-actions" style="margin-top:12px"><button class="btn btn-primary" onclick="addWish()">Add to my wishlist</button></div>
      <div id="myWishes" style="margin-top:14px"></div>` : `<div class="empty">Material sourcing is part of <b>Domestic Plus Membership</b> and above. <a href="#" onclick="go('membership');return false">Compare memberships</a></div>`}
    </div>`;
}
async function loadWishes() {
  const box = $('#myWishes'); if (!box) return;
  let d; try { d = await api('/api/wishlist'); } catch (e) { return; }
  const open = d.wishlist.filter((w) => w.status !== 'Closed');
  box.innerHTML = open.length ? `<h4 style="font-size:11px;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);margin-bottom:8px">Your wishlist</h4>${open.map((w) => `
    <div class="doc-row"><span class="t">${esc(w.category)}${w.itemType ? ' · ' + esc(w.itemType) : ''}${w.quantity ? ` · ${w.quantity} needed` : ''}<span>${esc(w.notes || '')}${w.status === 'Matched' ? ` · matched — see ${esc(w.matchedSku)}` : ''} · added ${fmtDate(w.createdAt.slice(0, 10))}</span></span>
      ${w.status === 'Matched' ? '<span class="pill pill-green">In stock</span>' : '<span class="pill pill-yellow">Watching</span>'} <button class="btn btn-ghost btn-sm" onclick="closeWish('${esc(w.id)}')">Remove</button></div>`).join('')}` : '';
}
async function addWish() {
  const body = { category: $('#wlCat').value.trim(), itemType: $('#wlType').value.trim(), quantity: $('#wlQty').value, notes: $('#wlNotes').value.trim() };
  if (!body.category && !body.itemType && !body.notes) return toast('Tell us what you’re looking for');
  try { await api('/api/wishlist', { method: 'POST', body }); toast('Added — we’ll email you when it comes in'); ['#wlCat', '#wlType', '#wlQty', '#wlNotes'].forEach((s) => { $(s).value = ''; }); loadWishes(); } catch (e) { toast(e.message); }
}
async function closeWish(id) { try { await api('/api/wishlist/' + id, { method: 'DELETE' }); loadWishes(); } catch (e) { toast(e.message); } }
// Buy / Reserve go through the marketplace checkout (Stripe) with the member's email locked in,
// so the order lands on this account and the payment is actually taken.
const checkoutUrl = (url) => url ? url + (url.includes('?') ? '&' : '?') + 'email=' + encodeURIComponent(state.user.email) : '';
function buyButtons(x, size = '') {
  if (!x.sku || state.user.role === 'admin') return '';
  const onSale = x.availability === 'Available' && (x.quantity || 0) > 0 && x.buyUrl;
  if (!onSale) return x.availability ? `<span class="pill pill-yellow">${esc(x.availability)}</span>` : '';
  const credit = CREDIT && CREDIT.credit ? CREDIT.credit.remaining : 0;
  const covered = credit > 0 && credit >= (x.listedPrice || 0); // at least one item is free with credit
  const qid = `qty-${x.id.replace(/[^\w]/g, '')}-${size ? 'c' : 'm'}`;
  return `<span class="qty-pick" onclick="event.stopPropagation()"><button type="button" onclick="qtyStep('${qid}',-1)">−</button><input id="${qid}" type="number" min="1" max="${x.quantity}" value="1" onchange="qtyClamp(this)"><button type="button" onclick="qtyStep('${qid}',1)">+</button></span>
    <button class="btn btn-primary ${size}" onclick="event.stopPropagation();buyItem('${esc(x.sku)}','buy','${qid}')">${covered ? 'Buy with credit' : 'Buy'}</button>
    ${covered ? '' : `<button class="btn btn-ghost ${size}" onclick="event.stopPropagation();buyItem('${esc(x.sku)}','deposit','${qid}')">Reserve</button>`}`;
}
function qtyClamp(i) { const max = +i.max || 1; i.value = Math.min(max, Math.max(1, Math.round(+i.value) || 1)); }
function qtyStep(id, d) { const i = $('#' + id); if (!i) return; i.value = (+i.value || 1) + d; qtyClamp(i); }
async function buyItem(sku, mode, qid) {
  const qty = qid && $('#' + qid) ? +$('#' + qid).value || 1 : 1;
  try {
    const r = await api('/api/orders/checkout', { method: 'POST', body: { sku, mode, qty } });
    if (r.url) { window.location.href = r.url; return; }
    if (r.order) { toast(`${r.order.id} — claimed with your membership credit`); closeModal(); go('orders'); }
...
function mpCard(x) {
  ...
  return `<div class="mp-card" onclick="openMaterial('${esc(x.id)}')">
    ...
    <div class="mp-body">
      ...
      <div class="mp-foot"><span class="item-carbon">${fmtKg(perItemKg(x))} CO₂e${Number(x.quantity) > 1 ? ' each' : ''}</span>${x.listedPrice ? `<span class="mp-price">${fmtGBP(x.listedPrice)} <span class="small muted">${esc(x.priceUnit || 'each')}</span>${x.retailNew > x.listedPrice ? `<span class="mp-new">≈ ${fmtGBP(x.retailNew)} new · save ${Math.round((1 - x.listedPrice / x.retailNew) * 100)}%</span>` : ''}</span>` : x.sourceProject ? `<span class="small muted">${esc(x.sourceProject)}</span>` : ''}</div>
      ${x.sku && state.user.role !== 'admin' ? `<div class="mp-buy">${buyButtons(x, 'btn-sm')}</div>` : ''}
    </div></div>`;
}
...
async function openMaterial(id) {
  ...
  <div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal modal-lg">
      ...
      <div class="passport-grid">
        ...
        <div class="f"><b>Carbon saved by reuse</b><span style="color:#1d7a05;font-weight:700">${fmtKg(perItemKg(x))} CO₂e${Number(x.quantity) > 1 ? ` each · ${fmtKg(x.carbonSavedKg || 0)} for all ${x.quantity}` : ''}${x.carbonEstimated ? ' <span class="pill pill-yellow" style="font-size:9px">estimated</span>' : ''}</span></div>
        ...
        <div class="f"><b>${x.listedPrice ? 'Price' : 'Indicative value'}</b><span>${x.listedPrice ? `${fmtGBP(x.listedPrice)} ${esc(x.priceUnit || 'each')}${x.retailNew > x.listedPrice ? ` <span class="muted small">· would cost ≈ ${fmtGBP(x.retailNew)} new (save ${Math.round((1 - x.listedPrice / x.retailNew) * 100)}%)</span>` : ''}` : x.valueGBP ? fmtGBP(x.valueGBP) : '—'}</span></div>
      ...
      ${(x.history || []).length ? `<h4 style="margin:4px 0 8px;font-size:11px;letter-spacing:.1em;text-transform:uppercase;color:var(--muted)">Provenance</h4>${historyHTML(x.history)}` : ''}
  ...
  if (isAdmin) scanActions(x);
}

/* scan-to-collect: an admin opening a passport (e.g. by scanning its QR on a phone) gets one-tap actions */
async function scanActions(x) {
  let d; try { d = await api('/api/admin/scan/' + x.id); } catch (e) { return; }
  const box = $('#scanActions'); if (!box) return;
  const arrived = x.status !== 'In storage' && !['Rehomed', 'Recycled'].includes(x.status) && !x.sku;
  if (!d.orders.length && !arrived && !(x.quantity > 0 && !['Rehomed', 'Recycled'].includes(x.status))) return;
  box.innerHTML = `<div class="card scan-card"><h4>On-site actions</h4>
    ${d.orders.map((o) => `<div class="scan-row"><div><b>${esc(o.id)}</b> — ${esc(o.member)} · ${o.qty || 1} × · ${esc(o.status)}${o.slot && !/to be confirmed/i.test(o.slot) ? ` · ${esc(o.slot)}` : ''}${o.balanceDueGBP && !o.balancePaid ? ` · <span class="pill pill-yellow">balance ${fmtGBP(o.balanceDueGBP)} due</span>` : ''}</div>
      <button class="btn btn-green btn-sm" onclick="scanCollect('${esc(o.id)}','${esc(x.id)}')">Mark collected</button></div>`).join('')}
    ${arrived ? `<div class="scan-row"><div>Not yet in the warehouse</div><button class="btn btn-primary btn-sm" onclick="scanArrived('${esc(x.id)}')">Arrived in warehouse</button></div>` : ''}
    ${x.quantity > 0 && !['Rehomed', 'Recycled'].includes(x.status) ? `<div class="scan-sell">
      <div class="small" style="margin-bottom:6px"><b>Walk-in sale</b> — paid in person. If the email already belongs to a member the sale goes on their account either way; otherwise choose whether to create one (they get a welcome email with a temporary password).</div>
      <div class="form-grid" style="grid-template-columns:2fr 1.4fr 70px auto;gap:8px;align-items:end">
        <div><label>Buyer email</label><input id="wsEmail" type="email" placeholder="name@example.com"></div>
        <div><label>Name (if new)</label><input id="wsName" placeholder="Optional"></div>
        <div><label>Qty</label><input id="wsQty" type="number" min="1" max="${x.quantity}" value="1"></div>
      </div>
      <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
        <button class="btn btn-green btn-sm" onclick="scanSell('${esc(x.id)}', true)">Mark collected &amp; create account</button>
        <button class="btn btn-ghost btn-sm" onclick="scanSell('${esc(x.id)}', false)">Mark collected — no account</button>
      </div></div>` : ''}
  </div>`;
}
async function scanSell(mid, createAccount) {
  const email = $('#wsEmail').value.trim(); if (!email) return toast('Enter the buyer’s email');
  try {
    const r = await api(`/api/admin/scan/${mid}/sell`, { method: 'POST', body: { email, name: $('#wsName').value.trim(), qty: +$('#wsQty').value || 1, createAccount } });
    toast(`${r.order.id} logged${r.member ? ` for ${r.member.name}` : ' (no account)'}${r.created ? ' — account created and welcome email sent' : ''}`); openMaterial(mid);
  } catch (e) { toast(e.message); }
}
async function scanCollect(oid, mid) {
  try { await api('/api/admin/orders/' + oid, { method: 'PATCH', body: { status: 'Collected' } }); toast(`${oid} marked collected`); openMaterial(mid); } catch (e) { toast(e.message); }
}
async function scanArrived(mid) {
  try { await api('/api/admin/materials/' + mid, { method: 'PATCH', body: { status: 'In storage' } }); toast('Marked as in the warehouse'); openMaterial(mid); } catch (e) { toast(e.message); }
}
function offerBack(id) {
  api('/api/materials/' + id).then(({ material: x }) => {
    state.offerPrefill = { passportId: x.id, name: x.name, category: x.category, qty: x.quantity, unit: x.unit, condition: x.condition };
    closeModal(); go('offer');
  }).catch((e) => toast(e.message));
}
...
async function materialModal(id, preset = {}) {
  ...
  <div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal modal-lg">
      ...
      <div id="mpStock" style="display:${ownerId ? 'none' : ''}">
        <div class="m-section"><h4>Marketplace listing</h4><span class="m-hint">MRG stock only — this is what members and the website see</span></div>
        <div class="form-grid" style="grid-template-columns:1fr 1fr 1fr 1fr">
          <div><label>Price (£ each)</label><input id="mpPrice" type="number" step="0.01" min="0" value="${x.listedPrice ?? ''}" placeholder="0 = not for sale"></div>
          <div><label>Cost new (£ each, approx.)</label><input id="mpRetail" type="number" step="1" min="0" value="${x.retailNew || ''}" placeholder="Shown to buyers"></div>
          <div><label>Availability</label><select id="mpAvail">${optList(['Available', 'Sold'], x.availability === 'Sold' ? 'Sold' : 'Available')}</select></div>
          <div><label>Delivery</label><select id="mpFulfil">${optList(['Collection Only', 'Available For Delivery'], x.fulfilment || 'Collection Only')}</select></div>
        </div>
        <button type="button" class="row-add" onclick="suggestListing()">Suggest price, carbon &amp; cost-new with AI</button>
        <div id="mpSuggest" class="small" style="margin-top:6px"></div>
        <p class="small muted" style="margin-top:6px">Saving with a price and quantity puts it on sale straight away (and mirrors it to Airtable); members on the wishlist for this category are emailed automatically.</p>
      </div>
...
async function suggestListing() {
  const box = $('#mpSuggest'); box.innerHTML = '<span class="muted">Thinking…</span>';
  try {
    const s = await api('/api/admin/suggest', { method: 'POST', body: { name: $('#mpName').value.trim(), category: $('#mpCat').value.trim(), condition: $('#mpCond').value, quantity: parseFloat($('#mpQty').value) || 1, unit: $('#mpUnit').value.trim() || 'items', description: $('#mpDesc').value.trim() } });
    box.innerHTML = `<div class="suggest"><b>Suggested:</b> price <b>${fmtGBP(s.price)}</b> each (range ${fmtGBP(s.priceLow)}–${fmtGBP(s.priceHigh)}) · carbon saved <b>${s.carbonKg} kg CO₂e</b> each · costs new ≈ <b>${fmtGBP(s.retailNew)}</b> each<br><span class="muted">${esc(s.reason)}${s.comparables ? ` · based on ${s.comparables} comparable item${s.comparables === 1 ? '' : 's'} in the warehouse` : ''}</span></div>`;
  } catch (e) { box.innerHTML = `<span style="color:#B42318">${esc(e.message)}</span>`; }
}
...
async function mpOwnerChanged() {
  if ($('#mpStock')) $('#mpStock').style.display = $('#mpOwner').value ? 'none' : '';
...
async function saveMaterial(id) {
  ...
  if (!body.userId) body.stock = { price: parseFloat($('#mpPrice').value) || 0, retailNew: parseFloat($('#mpRetail').value) || 0, availability: $('#mpAvail').value, fulfilment: $('#mpFulfil').value };
...
RENDER.project = async () => {
  ...
  $('#pageCrumb').textContent = `${isAdmin ? 'Admin' : 'Material Reuse Hub'} / Projects / ${p.name}`;
  ...
  $('#view').innerHTML = `
    ...
    <div style="display:flex;gap:10px;margin-top:18px;flex-wrap:wrap">
      ${isAdmin || (state.tier && state.tier.gates.impactCentre) ? `<button class="btn btn-primary" onclick="impactDoc({projectId:'${p.id}'})">Download impact report (PDF)</button>
...
RENDER.orders = async () => {
  await _ordersBase();
  if (state.tier.gates.audits) $('#view').insertAdjacentHTML('afterbegin', `<div class="tagline-strip" style="margin-bottom:16px"><span>Need a pre-refurbishment audit? It’s included in your partnership.</span>
      <button class="btn btn-green btn-sm" onclick="requestAudit()">Request an audit</button></div>`);
  const { requests, statuses } = await api('/api/requests');
  if (!requests.length) return;
  $('#view').insertAdjacentHTML('afterbegin', `
    <h3 class="section-title">Materials you’ve offered (${requests.length})</h3>${requests.map((r) => requestCard(r, statuses)).join('')}<h3 class="section-title" style="margin-top:24px">Warehouse orders</h3>`);
};
...
async function requestAudit() {
  const note = prompt('Tell us about the site (address, size, timescales) — optional:', '');
  if (note === null) return;
  try { await api('/api/audit-request', { method: 'POST', body: { note } }); toast('Request sent — your account manager will be in touch within one working day'); }
  catch (e) { toast(e.message); }
}
...
async function docModal(projectId = '', forUserId = null) {
  ...
  <div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal">
      ...
      <p class="m-sub">PDFs or photos (JPG, PNG, WebP, HEIC) up to 10 MB. Leave the file empty to log a document MRG holds on your behalf.</p>
      <div class="form-grid">
        ...
        <div style="grid-column:1/-1"><label>File</label><input id="dmFile" type="file" accept=".pdf,.png,.jpg,.jpeg,.webp,.heic" onchange="if(this.files[0]&&!$('#dmName').value)$('#dmName').value=this.files[0].name.replace(/\\.[^.]+$/,'')"></div>
      </div>
      <div class="modal-actions">
...
async function saveDoc(owner) {
  ...
  try {
    ...
    if (file) f = await uploadFile(file, owner, true); // documents are private — opened through short-lived links
...
RENDER.offer = async () => {
  ...
  const prefill = state.offerPrefill || null; state.offerPrefill = null;
  ...
  $('#view').innerHTML = `
    ...
    <div class="card" id="ofForm" style="margin-top:24px">
      ...
      ${prefill ? `<p class="small" style="margin:0 0 8px"><span class="pill pill-green">Passport ${esc(prefill.passportId)}</span> Offering this material back — its passport history will show the new chapter.</p><input type="hidden" id="ofPassport" value="${esc(prefill.passportId)}">` : ''}
      <div class="row-editor" id="ofRows">${prefill ? offerRow(m.categories, prefill) : offerRow(m.categories)}${offerRow(m.categories)}</div>
...
async function submitOffer() {
  ...
  if ($('#ofPassport')) body.passportIds = [$('#ofPassport').value];
  if (sel === '__new') {
...
RENDER.dashboard = async () => {
  ...
  const g = state.tier.gates;
  if (!g.passports) return; // domestic tiers: the base dashboard already shows everything relevant
  const [{ requests }, { total }] = await Promise.all([api('/api/requests'), api('/api/materials?scope=mine')]);
  const open = requests.filter((r) => !['Rehomed', 'Declined'].includes(r.status));
  $('#view').insertAdjacentHTML('afterbegin', `
    <div class="grid cols-3">
      <div class="card cta-card"><h3>Browse the marketplace</h3><p class="small">Every item in the warehouse with its passport — buy outright or reserve with a deposit, and it lands straight on your account.</p>
        <button class="btn btn-green btn-sm" onclick="MATF.scope='stock';go('materials')">Open the marketplace</button></div>
    ...
    <div id="badgeCard"></div>`);
  badgeCard();
};

/* "Material Reuse Partner" badge — unlocks once a member has impact on the ledger */
async function badgeCard() {
  let b; try { b = await api('/api/badge'); } catch (e) { return; }
  const box = $('#badgeCard'); if (!box) return;
  if (!b.available) { box.innerHTML = `<div class="card" style="margin-top:18px"><h3 style="margin-bottom:6px">Your Material Reuse Partner badge</h3><p class="small muted">${esc(b.reason)} Then you can put it on your website, tender documents or LinkedIn.</p></div>`; return; }
  const embed = `<a href="https://www.material-reuse.co.uk" target="_blank" rel="noopener"><img src="${b.url}" alt="Material Reuse Partner — verified reuse with Material Reuse Group" width="360" height="120"></a>`;
  box.innerHTML = `<div class="card" style="margin-top:18px;display:grid;grid-template-columns:auto 1fr;gap:20px;align-items:center">
    <div id="badgeSvg" style="line-height:0">${b.svg}</div>
    <div><h3 style="margin-bottom:6px">Your Material Reuse Partner badge</h3>
      <p class="small muted">Live figure — it updates itself as your impact grows. Put it on your website, in tenders or on LinkedIn.</p>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
        <button class="btn btn-primary btn-sm" onclick="downloadBadge()">Download PNG</button>
        <button class="btn btn-ghost btn-sm" onclick="navigator.clipboard.writeText(${JSON.stringify(embed).replace(/"/g, '&quot;')}).then(()=>toast('Embed code copied'))">Copy website embed code</button>
        <button class="btn btn-ghost btn-sm" onclick="navigator.clipboard.writeText('${esc(b.url)}').then(()=>toast('Image link copied'))">Copy image link</button>
      </div></div></div>`;
}
async function downloadBadge() {
  const svg = $('#badgeSvg svg'); if (!svg) return;
  ...
  const c = document.createElement('canvas'); c.width = 360 * scale; c.height = 120 * scale;
  c.getContext('2d').drawImage(img, 0, 0, c.width, c.height);
  const a = document.createElement('a'); a.href = c.toDataURL('image/png'); a.download = 'material-reuse-partner-badge.png'; a.click();
}
...
RENDER.adminOverview = async () => {
  ...
  strip.insertAdjacentHTML('afterend', `
    <div class="card" style="margin-top:18px;display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap">
      <div><h3 style="margin-bottom:4px">Marketplace</h3><p class="small" style="margin-bottom:4px">Stock is managed here: <b>Marketplace & Passports → New passport</b> (price, availability and delivery are in the listing section). Every item gets a passport and QR code; sales and reservations land on members’ accounts automatically.</p>
        <p class="small muted">${sync.enabled
          ? `Legacy Airtable sync on · every ${sync.intervalMinutes} min · last run ${sync.lastOk ? new Date(sync.lastOk).toLocaleString('en-GB') : 'not yet'}${sync.lastError ? ` · <span style="color:#B42318">last error: ${esc(sync.lastError)}</span>` : ''}`
          : 'Airtable / Softr sync off — the portal is the source of truth.'}
          ${sync.mail ? ' · Email notifications on' : ' · Email notifications off (set BREVO_API_KEY)'}</p></div>
    ...
    <div id="systemCard"></div>`);
  systemCard();
};

async function adminWishlist() {
  const { wishlist } = await api('/api/admin/wishlist');
  const open = wishlist.filter((w) => w.status !== 'Closed');
  $('#modalRoot').innerHTML = `<div class="modal-bg" onclick="if(event.target===this)closeModal()"><div class="modal modal-lg"><button class="close" onclick="closeModal()">✕</button>
    <h3>Wishlist requests (${open.length})</h3><p class="m-sub">Members are emailed automatically when a matching item is listed. Category matches exactly; keywords match the item name.</p>
    ${open.length ? `<table class="tbl small"><tr><th>Member</th><th>Looking for</th><th>Qty</th><th>Added</th><th>Status</th></tr>${open.map((w) => `<tr><td>${esc(w.member || w.name)}<br><span class="muted">${esc(w.email)}</span></td><td><b>${esc(w.category)}</b>${w.itemType ? ' · ' + esc(w.itemType) : ''}<br><span class="muted">${esc(w.notes || '')}</span></td><td>${w.quantity || '—'}</td><td>${fmtDate(w.createdAt.slice(0, 10))}</td><td>${w.status === 'Matched' ? `<span class="pill pill-green">matched ${esc(w.matchedSku || '')}</span>` : '<span class="pill pill-yellow">watching</span>'}</td></tr>`).join('')}</table>` : '<div class="empty">No open requests.</div>'}
  </div></div>`;
}
async function systemCard() {
  let d; try { d = await api('/api/admin/system?limit=40'); } catch (e) { return; }
  const box = $('#systemCard'); if (!box) return;
  const st = d.status;
  const on = (v, hint) => v ? '<span class="pill pill-green">on</span>' : `<span class="pill pill-yellow" title="${esc(hint)}">off</span>`;
  const embedCode = `<iframe src="${d.embed}" width="100%" height="150" frameborder="0" style="border:0;overflow:hidden" title="Material Reuse Group impact"></iframe>`;
  box.innerHTML = `
    <div class="grid cols-2" style="margin-top:16px">
      <div class="card"><h3 style="margin-bottom:6px">Automations</h3>
        <table class="tbl small"><tr><td>Stripe balance invoices</td><td>${on(st.stripe, 'set STRIPE_SECRET_KEY on Render')}</td></tr>
          <tr><td>SMS (Brevo or Twilio)</td><td>${on(st.sms, 'set BREVO_API_KEY (or TWILIO_*) on Render')}</td></tr>
          <tr><td>Wishlist alerts</td><td>${st.alertsSent} sent · <a href="#" onclick="adminWishlist();return false">view requests</a></td></tr>
          <tr><td>Weekly new-stock email</td><td>to members with “new stock” on · <a href="#" onclick="api('/api/admin/system/stockmail',{method:'POST'}).then((r)=>toast(r.sent?'Sent':'Nothing new this week'));return false">send now</a></td></tr>
          <tr><td>Stale holds</td><td>${st.staleDue} need a slot · reminder day ${st.staleRemindDays}, released day ${st.staleReleaseDays}</td></tr>
          <tr><td>Weekly digest</td><td>${esc(st.digest)} London · last sent ${st.lastDigestAt ? new Date(st.lastDigestAt).toLocaleString('en-GB') : 'never'}</td></tr></table>
        <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
          <button class="btn btn-ghost btn-sm" onclick="api('/api/admin/system/digest',{method:'POST'}).then(()=>toast('Digest sent to all admins'))">Send digest now</button>
          <button class="btn btn-ghost btn-sm" onclick="api('/api/admin/system/run',{method:'POST'}).then(()=>{toast('Automations run — alerts, invoice checks and stale holds');systemCard()})">Run automations now</button></div></div>
      <div class="card"><h3 style="margin-bottom:6px">Live impact counter for the website</h3>
        <p class="small muted">Total tonnes diverted, CO₂e saved and items rehomed — straight from the ledger, updates every minute. Paste this where you want it on material-reuse.co.uk.</p>
        <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap"><button class="btn btn-primary btn-sm" onclick="navigator.clipboard.writeText(${JSON.stringify(embedCode).replace(/"/g, '&quot;')}).then(()=>toast('Embed code copied — paste it into a Custom HTML block on the website'))">Copy embed code</button><a class="btn btn-ghost btn-sm" href="${esc(d.embed)}" target="_blank" rel="noopener">Preview</a>
        <button class="btn btn-ghost btn-sm" onclick="navigator.clipboard.writeText('<iframe src=&quot;${esc(location.origin)}/marketplace&quot; width=&quot;100%&quot; height=&quot;1400&quot; frameborder=&quot;0&quot; style=&quot;border:0&quot; title=&quot;Material Reuse Group marketplace&quot;></iframe>').then(()=>toast('Marketplace embed code copied'))">Copy marketplace embed code</button></div></div>
    </div>
    <div class="card" style="margin-top:16px"><h3 style="margin-bottom:6px">Audit log</h3><p class="small muted">Every change made by an admin, and every automatic action, newest first.</p>
      ${d.audit.length ? `<table class="tbl small" style="margin-top:8px"><tr><th>When</th><th>Who</th><th>Action</th><th>What</th><th>Detail</th></tr>
        ${d.audit.map((a) => `<tr><td>${new Date(a.ts).toLocaleString('en-GB', { dateStyle: 'short', timeStyle: 'short' })}</td><td>${esc(a.by)}</td><td>${esc(a.action)}</td><td>${esc(a.target)}</td><td class="muted">${esc(a.detail)}</td></tr>`).join('')}</table>` : '<div class="empty">Nothing yet.</div>'}</div>`;
}
...
RENDER.adminMember = async () => {
  ...
  $('#view').insertAdjacentHTML('beforeend', `
    ${u.tier === 'community' ? `<p class="small" style="margin-top:10px"><b>Registered installation address:</b> ${esc(u.address || '— not set (buying is blocked until it is)')}</p>${d.orders.filter((o) => o.evidence).map((o) => `<div class="card scan-card" style="margin-top:10px"><h4>Installation photos — ${esc(o.id)}</h4><div class="mp-thumbs">${o.evidence.photos.map((p) => `<img src="${esc(p.url)}" alt="" onclick="window.open('${esc(p.url)}','_blank')">`).join('')}</div>${o.evidence.note ? `<p class="small" style="margin-top:6px">${esc(o.evidence.note)}</p>` : ''}<p class="small muted" style="margin-top:6px">Uploaded ${fmtDate(o.evidence.at.slice(0, 10))} · ${esc(o.evidence.status)}${o.evidence.reviewedBy ? ' by ' + esc(o.evidence.reviewedBy) : ''}</p>${o.evidence.status === 'Submitted' ? `<div style="display:flex;gap:8px;margin-top:8px"><button class="btn btn-green btn-sm" onclick="api('/api/admin/orders/${esc(o.id)}/evidence',{method:'POST',body:{verified:true}}).then(()=>{toast('Verified');go('adminMember')})">Verify</button><button class="btn btn-ghost btn-sm" onclick="api('/api/admin/orders/${esc(o.id)}/evidence',{method:'POST',body:{verified:false}}).then(()=>{toast('Marked as not accepted');go('adminMember')})">Not acceptable</button></div>` : ''}</div>`).join('')}` : ''}
    ${u.stripeSubscriptionId || u.pendingTier ? `<p class="small muted" style="margin-top:10px">Membership: ${u.stripeSubscriptionId ? `paid by card via Stripe${u.tierRenews ? ', renews ' + fmtDate(u.tierRenews) : ''}` : ''}${u.pendingTier ? ` · changes to <b>${esc(u.pendingTier)}</b> on ${fmtDate(u.pendingAt)}` : ''}${u.paymentIssue ? ' · <span class="pill pill-red">payment failed</span>' : ''}</p>` : ''}
    <div class="grid cols-2" style="margin-top:16px">
      <div class="card">
...
editMember = function (id) {
  ...
  <div class="modal-bg" onclick="if(event.target===this)closeModal()"><div class="modal">
    ...
    <div class="form-grid">
      ...
      <div style="grid-column:1/-1"><label>Address${mm.tier === 'community' ? ' — registered installation address (required for Community; the member can’t change it)' : ''}</label><input id="emAddr" value="${esc(mm.address || '')}"></div>
...
/* =====================================================================
   PORTAL ASSISTANT — floating chat, answers from the portal + this member's own data
   ===================================================================== */
const CHAT = { open: false, messages: [], busy: false, on: null };
const chatSuggestions = () => state.user && state.user.role === 'admin'
  ? ['Which orders are waiting for a collection slot?', 'How do I log a walk-in sale?', 'What does the weekly digest include?', 'How do wishlist alerts work?']
  : state.tier && state.tier.gates.passports
    ? ['Where is my order and when can I collect?', 'How does Reserve and the deposit work?', 'What does my membership include?', 'How is my carbon saving worked out?', 'How do I download my invoice?']
    : ['Where is my order and when can I collect?', 'How does Reserve and the deposit work?', 'How does my materials credit work?', 'Can I get this delivered?', 'How do I download my invoice?'];
function mountChat() {
  if (!$('#chatRoot') || !state.user) return;
  $('#chatRoot').innerHTML = `<button class="chat-fab" id="chatFab" onclick="toggleChat()"><i></i>Ask MRG</button><div id="chatPanel"></div>`;
  if (CHAT.on === null) api('/api/chat').then((r) => { CHAT.on = !!r.on; }).catch(() => { CHAT.on = false; });
}
function toggleChat(force) {
  CHAT.open = force !== undefined ? force : !CHAT.open;
  renderChat();
}
const chatText = (t) => esc(t).replace(/(https?:\/\/[^\s)]+)/g, '<a href="$1" target="_blank" rel="noopener">$1</a>').replace(/\*\*(.+?)\*\*/g, '<b>$1</b>');
function renderChat() {
  const p = $('#chatPanel'); if (!p) return;
  if (!CHAT.open) { p.innerHTML = ''; return; }
  const first = state.user.name.split(' ')[0];
  p.innerHTML = `<div class="chat-panel">
    <div class="chat-head"><div><b>Material Reuse Hub assistant</b><span>${CHAT.on === false ? 'Not switched on yet' : state.tier && state.tier.gates.passports ? 'Knows your orders, slots, invoices and impact' : 'Knows your orders, slots, credit and invoices'}</span></div><button onclick="toggleChat(false)" aria-label="Close">✕</button></div>
    <div class="chat-log" id="chatLog">
      <div class="chat-msg bot">Hi ${esc(first)} — ask me anything about your orders, collections, membership${state.tier && state.tier.gates.passports ? ', passports or your impact. For anything I can\'t sort, I\'ll point you to the right person.' : ' or credit. Anything I can\'t sort myself, I\'ll pass to the team and they\'ll email you.'}</div>
      ${CHAT.messages.map((m) => `<div class="chat-msg ${m.role === 'user' ? 'user' : 'bot'}">${chatText(m.content)}</div>`).join('')}
      ${CHAT.busy ? '<div class="chat-msg bot typing">Thinking…</div>' : ''}
    </div>
    ${!CHAT.messages.length ? `<div class="chat-sugg">${chatSuggestions().map((q) => `<button onclick="sendChat(${JSON.stringify(q).replace(/"/g, '&quot;')})">${esc(q)}</button>`).join('')}</div>` : ''}
    <form class="chat-form" onsubmit="event.preventDefault();sendChat()">
      <textarea id="chatInput" placeholder="Type a question…" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();sendChat()}"></textarea>
      <button class="btn btn-primary btn-sm" type="submit" ${CHAT.busy ? 'disabled' : ''}>Send</button>
    </form></div>`;
  const log = $('#chatLog'); if (log) log.scrollTop = log.scrollHeight;
  const inp = $('#chatInput'); if (inp && !CHAT.busy) inp.focus();
}
async function sendChat(text) {
  const inp = $('#chatInput');
  const q = (text || (inp ? inp.value : '')).trim();
  if (!q || CHAT.busy) return;
  CHAT.messages.push({ role: 'user', content: q }); CHAT.busy = true; renderChat();
  try {
    const r = await api('/api/chat', { method: 'POST', body: { messages: CHAT.messages.slice(-12) } });
    CHAT.messages.push({ role: 'assistant', content: r.reply });
  } catch (e) { CHAT.messages.push({ role: 'assistant', content: `Sorry — ${e.message}. Please try again in a moment.` }); }
  CHAT.busy = false; renderChat();
}
const _enterAppChat = enterApp;
enterApp = function (user, tier) { _enterAppChat(user, tier); CHAT.messages = []; CHAT.open = false; mountChat(); };
const _logoutChat = typeof logout === 'function' ? logout : null;
if (_logoutChat) logout = function () { CHAT.messages = []; CHAT.open = false; if ($('#chatRoot')) $('#chatRoot').innerHTML = ''; return _logoutChat(); };
