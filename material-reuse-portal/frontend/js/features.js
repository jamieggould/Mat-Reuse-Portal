...
RENDER.materials = async () => {
  ...
  const simple = !isAdmin && !state.tier.gates.passports; // domestic tiers: the Reuse Hub is the shop
...
function mpCard(x) {
  ...
  return `<div class="mp-card" onclick="openMaterial('${esc(x.id)}')">
    ...
    <div class="mp-body">
      ...
      <div class="mp-foot"><span class="item-carbon">${fmtKg(x.carbonSavedKg || 0)} CO₂e</span>${x.listedPrice ? `<span class="mp-price">${fmtGBP(x.listedPrice)} <span class="small muted">${esc(x.priceUnit || 'each')}</span></span>` : x.sourceProject ? `<span class="small muted">${esc(x.sourceProject)}</span>` : ''}</div>
  ...
  <div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal modal-lg">
      ...
      <div id="mpStock" style="display:${ownerId ? 'none' : ''}">
        ...
        <p class="small muted" style="margin-top:6px">Saving with a price and quantity puts it on sale straight away; members on the wishlist for this category are emailed automatically.</p>
      </div>
...
async function saveMaterial(id) {
  ...
  if (!body.userId) body.stock = { price: parseFloat($('#mpPrice').value) || 0, availability: $('#mpAvail').value, fulfilment: $('#mpFulfil').value };
