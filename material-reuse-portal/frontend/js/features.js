...
function mpCard(x) {
  ...
  return `<div class="mp-card" onclick="openMaterial('${esc(x.id)}')">
    ...
    <div class="mp-body">
      ...
      <div class="mp-foot"><span class="item-carbon">${fmtKg(x.carbonSavedKg || 0)} CO₂e</span>${x.listedPrice ? `<span class="mp-price">${fmtGBP(x.listedPrice)} <span class="small muted">${esc(x.priceUnit || 'each')}</span>${x.retailNew > x.listedPrice ? `<span class="mp-new">≈ ${fmtGBP(x.retailNew)} new · save ${Math.round((1 - x.listedPrice / x.retailNew) * 100)}%</span>` : ''}</span>` : x.sourceProject ? `<span class="small muted">${esc(x.sourceProject)}</span>` : ''}</div>
