...
const qrUrl = (data) => `https://api.qrserver.com/v1/create-qr-code/?size=180x180&margin=4&data=${encodeURIComponent(data)}`;
...
<div class="sheet">
  <div class="head"><div class="lockup">${logoSVG}<div class="wordmark">material<br>reuse</div></div>
  ...
  <div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal modal-lg">
      ...
      <div class="passport-grid">
        ...
        <div class="f"><b>Dimensions</b><span>${esc(x.dimensions || '—')}</span></div>
        ...
        <div class="f"><b>Carbon saved by reuse</b><span style="color:#1d7a05;font-weight:700">${fmtKg(x.carbonSavedKg || 0)} CO₂e</span></div>
        ${x.weightKg ? `<div class="f"><b>Weight diverted</b><span>${fmtT(x.weightKg)}</span></div>` : ''}
      ...
      <div class="qr-box"><img class="qr-img" src="${qrUrl(passportLink(x.id))}" alt="QR code" width="88" height="88">
        <div><b style="font-family:var(--font-head)">${esc(x.ref)}</b><br>
        <span class="small" style="color:#B9C6E4">Scan on site to open this passport. ${esc(passportLink(x.id))}</span></div></div>
      <div style="display:flex;gap:10px;margin-top:18px;flex-wrap:wrap">
        <button class="btn btn-primary" onclick="passportDoc('${esc(x.id)}')">Download passport (PDF)</button>
        ${x.sku && !isAdmin ? `<button class="btn btn-green" onclick="closeModal();go('warehouse')">See it in the warehouse</button>` : ''}
...
async function passportDoc(id) {
  ...
  brandDoc({
    title: `MRG Material Passport — ${x.ref}`, kicker: x.passportVerified ? 'Verified material passport' : 'Digital material passport',
    ...
    body: `
      <div class="cols" style="grid-template-columns:1.2fr 1fr;align-items:start">
        ...
        <div>
          ...
          <div class="qr"><img src="${qrUrl(passportLink(x.id))}" alt="QR"><div><b>${esc(x.ref)}</b><div class="small" style="margin-top:4px">Scan to open the live passport<br>${esc(passportLink(x.id))}</div></div></div>
        </div>
      </div>
      <div class="sec-t">Specification</div>
      <div class="kv">
        <div><b>Quantity</b><span>${x.quantity} ${esc(x.unit || 'items')}</span></div>
        <div><b>Dimensions</b><span>${esc(x.dimensions || '—')}</span></div>
      ...
      <div class="verify" style="margin-top:8mm">${x.passportVerified
        ...
        : 'This passport is being finalised. Figures are indicative until Material Reuse Group marks the passport as verified.'}</div>`,
  });
}
  ...
  <div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal modal-lg">
      ...
      <div class="form-grid">
        ...
        <div><label>Dimensions</label><input id="mpDims" value="${esc(x.dimensions || '')}" placeholder="e.g. 600 × 600 mm"></div>
      ...
      <div class="m-section"><h4>Impact & value</h4><span class="m-hint">Counted when the passport is marked Rehomed / Recycled</span></div>
      <div class="form-grid" style="grid-template-columns:1fr 1fr 1fr 1fr">
        <div><label>CO₂e saved (kg)</label><input id="mpCo2" type="number" step="0.1" min="0" value="${x.carbonSavedKg || 0}"></div>
        <div><label>Weight (kg)</label><input id="mpW" type="number" step="0.1" min="0" value="${x.weightKg || 0}"></div>
...
async function saveMaterial(id) {
  const body = {
    ...
    dimensions: $('#mpDims').value.trim(), userId: $('#mpOwner').value || null, projectId: $('#mpProject').value || null,
...
RENDER.project = async () => {
  ...
  $('#view').innerHTML = `
    <div class="tagline-strip">
      ...
      <button class="btn btn-green btn-sm" onclick="go('${isAdmin ? 'adminMember' : 'projects'}')">Back to projects</button>
    </div>
...
async function evidencePack(pid) {
  ...
  const p = d.project, t = d.impact.totals;
  brandDoc({
    title: `MRG Evidence Pack — ${p.name}`, kicker: 'Project evidence pack', heading: p.name,
...
RENDER.offer = async () => {
  ...
  $('#view').innerHTML = `
    ...
    ${requests.length ? `<h3 class="section-title" style="margin-top:24px">Your material requests</h3>
      ${requests.map((r) => requestCard(r, statuses)).join('')}` : ''}
...
async function impactDoc(filters = IMPF) {
  ...
  brandDoc({
    title: `MRG Impact & ESG Report — ${u.organisation || u.name}`, kicker: imp.meta.verified ? 'Independently verified' : 'Impact & ESG reporting',
...
RENDER.dashboard = async () => {
  ...
  strip.insertAdjacentHTML('afterend', `
    <div class="grid cols-3" style="margin-top:18px">
      ...
      <div class="card"><h3 style="margin-bottom:8px">Material requests</h3>
        ${open.length ? open.slice(0, 3).map((r) => `<div class="doc-row"><span class="t">${esc(r.id)}<span>${esc(r.projectName || 'New project')}</span></span>${statusPill(r.status)}</div>`).join('') : '<div class="empty" style="padding:14px 0">No open requests.</div>'}</div>
...
RENDER.adminMember = async () => {
  ...
  $('#view').insertAdjacentHTML('beforeend', `
    ...
    <div class="grid cols-2" style="margin-top:16px">
      <div class="card">
        <div class="order-head"><h3>Material requests</h3><button class="btn btn-ghost btn-sm" onclick="go('adminRequests')">Open queue</button></div>
