...
(function wireNav() {
  ...
  ['projects', 'lists', 'warehouse', 'offer'].forEach((id) => { const i = PAGES.findIndex((p) => p.id === id); if (i >= 0) PAGES.splice(i, 1); });
...
function wishlistCard() {
  return `<div class="card" id="wishlistCard" style="margin-top:28px">
      ...
      <p class="small muted" style="margin-bottom:14px">Tell us what you’re after. Kallie sees every request, and you’re emailed${state.user.phone ? ' and texted' : ''} the moment a matching item is listed.</p>
...
RENDER.adminMember = async () => {
  ...
  $('#view').insertAdjacentHTML('beforeend', `
    ${u.communityRequest ? `<div class="tagline-strip" style="margin-top:16px"><span><b>Community Free Membership request</b> — ${esc(u.communityRequest.organisation)} · ${fmtDate(u.communityRequest.at.slice(0, 10))}</span>
      <span style="display:flex;gap:8px"><button class="btn btn-green btn-sm" onclick="decideCommunity('${u.id}',true)">Approve</button><button class="btn btn-ghost btn-sm" style="border-color:#fff;color:#fff" onclick="decideCommunity('${u.id}',false)">Decline</button></span></div>` : ''}
...
async function decideCommunity(id, approve) {
  const note = approve ? '' : (prompt('Reason (sent to the member, optional):', '') || '');
  try { await api(`/api/admin/members/${id}/community`, { method: 'POST', body: { approve, note } }); toast(approve ? 'Approved — member emailed' : 'Declined — member emailed'); go('adminMember'); } catch (e) { toast(e.message); }
}
