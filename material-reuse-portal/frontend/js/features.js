    ...
    <div class="grid cols-2" style="margin-top:16px">
      <div class="card"><h3 style="margin-bottom:6px">Automations</h3>
        <table class="tbl small"><tr><td>Stripe balance invoices</td><td>${on(st.stripe, 'set STRIPE_SECRET_KEY on Render')}</td></tr>
          <tr><td>SMS (Twilio)</td><td>${on(st.sms, 'set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM on Render')}</td></tr>
...
function renderChat() {
  ...
  p.innerHTML = `<div class="chat-panel">
    <div class="chat-head"><div><b>Marketplace assistant</b><span>${CHAT.on === false ? 'Not switched on yet' : state.tier && state.tier.gates.passports ? 'Knows your orders, slots, invoices and impact' : 'Knows your orders, slots, credit and invoices'}</span></div><button onclick="toggleChat(false)" aria-label="Close">✕</button></div>
