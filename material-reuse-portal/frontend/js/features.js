...
async function scanActions(x) {
  ...
  box.innerHTML = `<div class="card scan-card"><h4>On-site actions</h4>
    ...
    ${x.quantity > 0 && !['Rehomed', 'Recycled'].includes(x.status) ? `<div class="scan-sell">
      <div class="small" style="margin-bottom:6px"><b>Walk-in sale</b> — paid in person. The buyer gets an account (if new) and this appears on their orders.</div>
      <div class="form-grid" style="grid-template-columns:2fr 1.4fr 70px auto;gap:8px;align-items:end">
        ...
        <button class="btn btn-green btn-sm" onclick="scanSell('${esc(x.id)}')">Mark collected</button>
      </div></div>` : ''}
  </div>`;
}
async function scanSell(mid) {
  const email = $('#wsEmail').value.trim(); if (!email) return toast('Enter the buyer’s email');
  try {
    const r = await api(`/api/admin/scan/${mid}/sell`, { method: 'POST', body: { email, name: $('#wsName').value.trim(), qty: +$('#wsQty').value || 1 } });
    toast(`${r.order.id} logged for ${r.member.name}${r.created ? ' — account created and welcome email sent' : ''}`); openMaterial(mid);
  ...
  box.innerHTML = `
    <div class="grid cols-2" style="margin-top:16px">
      ...
      <div class="card"><h3 style="margin-bottom:6px">Live impact counter for the website</h3>
        ...
        <textarea readonly style="width:100%;height:64px;font-family:monospace;font-size:11px;margin-top:8px" onclick="this.select()">${esc(embedCode)}</textarea>
        <div style="display:flex;gap:8px;margin-top:8px"><button class="btn btn-ghost btn-sm" onclick="navigator.clipboard.writeText(${JSON.stringify(embedCode).replace(/"/g, '&quot;')}).then(()=>toast('Embed code copied'))">Copy</button><a class="btn btn-ghost btn-sm" href="${esc(d.embed)}" target="_blank" rel="noopener">Preview</a></div></div>
    </div>
