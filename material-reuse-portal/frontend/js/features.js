...
(function wireNav() {
  ...
  after(PAGES, 'warehouse', [
    { id: 'materials', label: 'Marketplace & Passports' },
  ]);
...
function buyButtons(x, size = '') {
  ...
  if (onSale) return `<button class="btn btn-primary ${size}" onclick="event.stopPropagation();window.open('${esc(checkoutUrl(x.buyUrl))}','_blank')">Buy</button>${x.reserveUrl ? `<button class="btn btn-ghost ${size}" onclick="event.stopPropagation();window.open('${esc(checkoutUrl(x.reserveUrl))}','_blank')">Reserve</button>` : ''}`;
  return x.availability ? `<span class="pill pill-yellow">${esc(x.availability)}</span>` : '';
}
function matFilter() {
