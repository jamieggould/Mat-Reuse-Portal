  ...
  <div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal modal-lg">
      ...
      <div style="display:flex;gap:10px;margin-top:18px;flex-wrap:wrap">
        ...
        ${!isAdmin && x.userId === state.user.id && ['Rehomed', 'Recovered', 'In storage'].includes(x.status) ? `<button class="btn btn-ghost" onclick="offerBack('${esc(x.id)}')">Offer this back for reuse</button>` : ''}
...
async function scanActions(x) {
  ...
  if (!d.orders.length && !arrived) return;
