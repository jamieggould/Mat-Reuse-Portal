...
The portal is called the Marketplace. MEMBERSHIPS (compare, upgrade and downgrade on the Membership & Billing page):
${tiers}
  ...
  async function reply(actor, messages) {
    ...
    if (esc_) { // hand the thread to the team without the member having to contact anyone
      ...
      integrations.sendMail(integrations.ENV.ADMIN_EMAIL, `Assistant hand-off — ${actor.name}`, 'A member needs the team',
        `<p><b>${actor.name}</b> (${actor.email}${actor.phone ? `, ${actor.phone}` : ''} · ${(tierOf(actor) || {}).name || actor.tier}) asked the Marketplace assistant for something it couldn't do.</p><p><b>Needed:</b> ${esc_[1].trim()}</p><pre style="font-size:12px;white-space:pre-wrap;background:#F5F6F9;padding:12px">${transcript.replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]))}</pre><p>Please email the member directly.</p>`,
