...
const clientIp = (req) => {
  ...
  const hops = xf.split(',').map((x) => x.trim()).filter(Boolean);
  return (hops.length ? hops[hops.length - 1] : req.socket.remoteAddress || '').replace(/^::ffff:/, ''); // last hop = what the proxy itself saw
};
...
const readBody = (req, limit) =>
  new Promise((resolve) => {
    ...
    req.on('end', () => { if (over) { req._body = {}; req._tooLarge = true; return resolve({}); } try { const v = raw ? JSON.parse(raw) : {}; req._body = v && typeof v === 'object' && !Array.isArray(v) ? v : {}; } catch { req._body = {}; } resolve(req._body); });
