...
module.exports = function install({ db, saveSessions }) {
  ...
  const SESSIONS = {
    ...
    set(t, uid, isAdmin) { db.sessions[t] = { uid, exp: Date.now() + (isAdmin ? ADMIN_SESSION_HOURS * 3600e3 : SESSION_DAYS * 864e5), at: Date.now() }; SESSIONS.sweep(); saveSessions(); },
    delete(t) { delete db.sessions[t]; saveSessions(); },
    deleteUser(uid, keep) { for (const t of Object.keys(db.sessions)) if (db.sessions[t].uid === uid && t !== keep) delete db.sessions[t]; saveSessions(); },
  ...
  const clientIp = (req) => (String(req.headers['x-forwarded-for'] || '').split(',')[0] || req.socket.remoteAddress || '').trim().replace(/^::ffff:/, '');
