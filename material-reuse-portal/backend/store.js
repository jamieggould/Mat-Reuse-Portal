...
const db = {
  ...
  system: (() => { try { return load('system.json').system; } catch (e) { return {}; } })(),
  wishlist: (() => { try { return load('wishlist.json').wishlist; } catch (e) { return []; } })(), // automation state: alert queue, audit log, digest marker
};
const PERSISTED = ['users', 'inventory', 'orders', 'lists', 'carbon', 'projects', 'materials', 'documents', 'requests', 'impactEvents', 'sessions', 'system', 'wishlist'];
...
function persist(key, fileBody) {
  ...
  if (SUPA) {
    fetch(`${SUPA_URL}/rest/v1/portal_data`, {
      ...
      body: JSON.stringify([{ key, body: db[key], updated_at: new Date().toISOString() }]),
...
const saveWishlist = () => persist('wishlist', { wishlist: db.wishlist });
...
module.exports = { DATA_DIR, load, db, PERSISTED, SUPA, SUPA_URL, supaHeaders, supaLoad, persist,
  saveUsers, saveCarbon, saveOrders, saveLists, saveProjects, saveInventory, saveSessions, saveSystem, saveWishlist, startBackups };
