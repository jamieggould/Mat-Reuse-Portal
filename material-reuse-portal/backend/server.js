...
async function api(req, res, url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/inventory') {
    ...
    if (s) items = items.filter((i) =>
      (i.title + i.sku + i.category + i.sentFrom).toLowerCase().includes(s));
    return json(res, 200, {
      items,
  ...
  if (req.method === 'POST' && url.pathname === '/api/orders') {
    ...
    if (!gates.reservations)
      return json(res, 403, { error: 'Reservations require Domestic Plus or above.' });
    ...
    if (!lines.length) return json(res, 400, { error: 'No valid items' });
    ...
    const order = {
      ...
      status: 'Reserved',
      fulfilment: body.fulfilment || 'Collection — Central London facility',
  ...
  if (req.method === 'POST' && url.pathname === '/api/lists') {
    ...
    if (limit !== null && count >= limit)
      return json(res, 403, { error: `Domestic Free includes ${limit} shopping list. Upgrade for unlimited lists.` });
