...
const { SESSIONS, FAILS, noteFail, tooMany, clientIp, weakPassword, PW_RULE, makeAuth, checkPw, safeUser } = require('./auth')({ db, saveSessions });
...
const readBody = (req) =>
  new Promise((resolve) => {
    ...
    req.on('data', (c) => (raw += c));
    req.on('end', () => {
      try { req._body = raw ? JSON.parse(raw) : {}; } catch { req._body = {}; }
      resolve(req._body);
    });
  });
...
const commerce = require('./commerce')({ db, features, integrations, automations, saveOrders, saveUsers, saveInventory, saveSystem, saveWishlist, nextOrderSeq: () => orderSeq++, safeUser });
...
async function api(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/forgot') {
    ...
    if (tooMany(ipKey)) return json(res, 429, { error: 'Too many requests — please wait 15 minutes.' });
    noteFail(ipKey);
  ...
  if (req.method === 'GET' && url.pathname === '/api/checkout') {
    try {
      ...
      return res.end(commerce.page('Not available', `<h1>${e.code === 'unavailable' ? 'Sorry, that one’s gone' : 'Checkout unavailable'}</h1><p>${e.message}</p><a class="btn" href="/marketplace">Back to the marketplace</a>`));
    }
  }
  ...
  if (req.method === 'GET' && parts[1] === 'public' && parts[2] === 'passports' && parts[3]) {
    ...
    if (!m.userId) { const inv = db.inventory.find((i) => i.sku === m.sku); if (inv) { out.listedPrice = inv.price; out.priceUnit = inv.priceUnit; out.availability = inv.status; } }
  ...
  if (req.method === 'POST' && parts[1] === 'admin' && parts[2] === 'scan' && parts[3] && parts[4] === 'sell') {
    ...
    try { const r = await automations.walkInSale(mat, { email, name: b.name, phone: b.phone, qty: b.qty }, actor); return json(res, 201, { order: r.order, member: safeUser(r.user), created: r.created }); }
  ...
  if (req.method === 'POST' && url.pathname === '/api/auth/password') {
    ...
    if (weakPassword(body.next)) return json(res, 400, { error: PW_RULE });
  ...
  if (req.method === 'POST' && url.pathname === '/api/wishlist') {
    const b = await readBody(req);
    if (!tierOf(actor) || !tierOf(actor).gates.materialSourcing) return json(res, 403, { error: 'Material sourcing (the wishlist) is part of Domestic Plus Membership and above.' });
  ...
  if (parts[1] === 'admin') {
    ...
    if (req.method === 'POST' && parts[2] === 'members' && parts[3] && parts[4] === 'community') {
      const u = userById(parts[3]); if (!u) return json(res, 404, { error: 'User not found' });
      const b = await readBody(req);
      try { await commerce.decideCommunity(u, !!b.approve, b.note, actor); return json(res, 200, { user: safeUser(u) }); } catch (e) { return json(res, 400, { error: e.message }); }
    }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (/^\/(order-complete|membership-complete|marketplace)\/?$/.test(url.pathname)) return await api(req, res, url); // commerce pages
