...
async function api(req, res, url) {
  ...
  // ---- buying from inside the Reuse Hub (quantity chosen here; membership credit applied automatically) ----
  ...
  if (req.method === 'POST' && parts[1] === 'orders' && parts[2] && parts[3] === 'evidence') {
    ...
    const b = await readBody(req);
    const photos = (b.photos || []).filter((p) => p && p.url).map((p) => ({ url: p.url, key: p.key || null, name: p.name || 'photo' }));
  ...
  if (req.method === 'PATCH' && parts[1] === 'users' && parts[2]) {
    ...
    ['name', 'phone', 'address', 'organisation'].forEach((k) => {
      if (body[k] !== undefined && !(k === 'address' && u.tier === 'community' && !isAdmin)) u[k] = body[k]; // community address is set by MRG only
    });
