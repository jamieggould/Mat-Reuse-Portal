...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;background:#fff;border:1px solid #DDE2EC">
  ...
  const mail = {
    welcome: (u, tempPassword) => sendMail(u.email, 'Welcome to the Material Reuse member portal', `Welcome, ${u.name.split(' ')[0]}`,
      ...
      portalLink('Sign in')),
    resetLink: (u, token) => sendMail(u.email, 'Reset your Material Reuse portal password', 'Reset your password',
  ...
  return { ENV, mail, alertAdmin, sync, status: () => ({ ...status, autoCreateMembers: ENV.AIRTABLE_AUTO_CREATE_MEMBERS, intervalMinutes: ENV.AIRTABLE_SYNC_MINUTES, mail: mailOn, adminEmail: ENV.ADMIN_EMAIL }),
