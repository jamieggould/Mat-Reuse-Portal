...
const ENV = {
  ...
  AIRTABLE_SYNC: process.env.AIRTABLE_SYNC === 'true', // legacy: the portal is now the source of truth; set AIRTABLE_SYNC=true only if Softr/Airtable must stay in step
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;background:#fff;border:1px solid #DDE2EC">
  ...
  return { ENV, mail, sendMail, alertAdmin, sync, status: () => ({ ...status, autoCreateMembers: ENV.AIRTABLE_AUTO_CREATE_MEMBERS, intervalMinutes: ENV.AIRTABLE_SYNC_MINUTES, mail: mailOn, adminEmail: ENV.ADMIN_EMAIL }),
