...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;background:#fff;border:1px solid #DDE2EC">
  ...
  function start() {
    console.log(`  integrations: airtable pull ${atOn ? `on (every ${ENV.AIRTABLE_SYNC_MINUTES} min)` : 'off'} · airtable push ${pushOn ? 'on' : 'off — set AIRTABLE_TOKEN'} · email ${mailOn ? 'on' : 'off — set RESEND_API_KEY'}`);
    if (atOn) { sync(); setInterval(sync, ENV.AIRTABLE_SYNC_MINUTES * 60 * 1000).unref(); }
  }

  return { ENV, mail, sendMail, alertAdmin, sync, pushListing, pushListingNow, pushStatus: () => pushStatus,
