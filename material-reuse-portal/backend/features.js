...
const { today, num, r1 } = require('./util');
...
module.exports = function install(ctx) {
  ...
  // Provenance chain: every passport keeps a permanent, append-only history of where it has been
  function addHistory(mat, event, detail, date) {
    mat.history = mat.history || [];
    const last = mat.history[mat.history.length - 1];
    if (last && last.event === event && last.detail === detail) return; // no duplicate consecutive entries
    mat.history.push({ date: date || today(), event, detail: detail || '', at: new Date().toISOString() });
  }
  ...
  function applyOrderStats(order) {
    ...
    items.forEach((l) => { const mat = l.sku && db.materials.find((x) => x.sku === l.sku || x.id === l.sku); if (mat) { addHistory(mat, 'Collected', `${Math.max(1, Math.round(num(l.qty, 1)))} ${mat.unit || 'items'} collected by a member${order.projectId && projectById(order.projectId) ? ' for a project' : ''} (${order.id})`); saveMaterials(); } });
  ...
  function migrate() {
    ...
    // 5. Provenance history for passports created before it existed
    db.materials.forEach((m) => {
      if ((m.history || []).length) return;
      addHistory(m, 'Recovered', m.sourceBuilding ? `Recovered from ${m.sourceBuilding}` : (m.sku ? 'Recovered into Material Reuse Group stock' : 'Recovered'), m.dateRecovered);
      if (m.sku && !['Recovered'].includes(m.status)) addHistory(m, 'Listed', 'Listed on the Material Reuse Group marketplace', m.dateRecovered);
      if (['Rehomed', 'Recycled', 'Reserved', 'In storage'].includes(m.status) && m.status !== 'Listed') addHistory(m, m.status, m.status === 'Rehomed' ? `Rehomed${m.reuseDestination ? ` — ${m.reuseDestination}` : ''}` : m.status === 'Recycled' ? 'Sent for recycling' : m.status === 'Reserved' ? 'Reserved by a member' : 'In the warehouse', m.dateRehomed || undefined);
      changed.add('materials');
    });
  ...
  const okUrl = (u) => security.safeFileUrl(u, SUPA_URL);
  const cleanPhotos = (arr, max = 12) => (Array.isArray(arr) ? arr : []).filter((p) => p && okUrl(p.url)).slice(0, max).map((p) => ({ url: p.url, key: safeKey(p.key), name: String(p.name || 'photo').slice(0, 120) }));
  // Only real images and PDFs are accepted, judged by their bytes not the file name
  const sniff = (buf) => {
    if (buf.length > 3 && buf[0] === 0xff && buf[1] === 0xd8 && buf[2] === 0xff) return 'image/jpeg';
    if (buf.length > 7 && buf[0] === 0x89 && buf[1] === 0x50 && buf[2] === 0x4e && buf[3] === 0x47) return 'image/png';
    if (buf.length > 5 && buf.slice(0, 4).toString() === 'GIF8') return 'image/gif';
    ...
    if (buf.length > 11 && buf.slice(4, 8).toString() === 'ftyp' && /hei[cf]|mif1/.test(buf.slice(8, 12).toString())) return 'image/heic';
    return null;
  };

  const PRIVATE_BUCKET = 'portal-private'; let privateBucketReady = false;
  async function ensurePrivateBucket() { // created once, private (no public URLs) — the service key reads it on the member's behalf
    if (privateBucketReady || !SUPA) return;
    try { const r = await fetch(`${SUPA_URL}/storage/v1/bucket`, { method: 'POST', headers: { ...supaHeaders, 'Content-Type': 'application/json' }, body: JSON.stringify({ id: PRIVATE_BUCKET, name: PRIVATE_BUCKET, public: false }) }); if (r.ok || r.status === 409 || r.status === 400) privateBucketReady = true; } catch (e) { /* retried next upload */ }
  }
  async function storeFile(ownerId, name, mime, buf, priv = false) {
    const key = `${priv ? 'private/' : ''}${ownerId}/${Date.now()}-${safeName(name)}`;
    if (SUPA && priv) {
      await ensurePrivateBucket();
      const res = await fetch(`${SUPA_URL}/storage/v1/object/${PRIVATE_BUCKET}/${key.replace(/^private\//, '')}`, {
        method: 'POST', headers: { apikey: supaHeaders.apikey, Authorization: supaHeaders.Authorization, 'Content-Type': mime || 'application/octet-stream', 'x-upsert': 'true' }, body: buf });
      if (!res.ok) throw new Error(`Storage upload failed (${res.status}): ${(await res.text()).slice(0, 200)}`);
      return { url: `/api/files/${key}`, key, private: true };
    }
    if (SUPA) {
    ...
    return priv ? { url: `/api/files/${key}`, key, private: true } : { url: `/uploads/${key}`, key };
  }
  // Private files: short-lived signed links (10 min) so a plain <a href> works without exposing anything permanent
  const fileSecret = () => { const sys = db.system; if (!sys.fileSecret) { sys.fileSecret = crypto.randomBytes(32).toString('hex'); ctx.saveSystem && ctx.saveSystem(); } return sys.fileSecret; };
  const signFile = (key, ttlMs = 60 * 60e3) => { const exp = Date.now() + ttlMs; const sig = crypto.createHmac('sha256', fileSecret()).update(`${key}|${exp}`).digest('hex').slice(0, 40); return `/api/files/${key}?exp=${exp}&sig=${sig}`; };
  const fileSigOk = (key, exp, sig) => { if (!exp || !sig || +exp < Date.now()) return false; const want = crypto.createHmac('sha256', fileSecret()).update(`${key}|${exp}`).digest('hex').slice(0, 40); return want.length === String(sig).length && crypto.timingSafeEqual(Buffer.from(want), Buffer.from(String(sig))); };
  async function readPrivateFile(key) { // → { buf, mime } or null
    if (SUPA) { const r = await fetch(`${SUPA_URL}/storage/v1/object/${PRIVATE_BUCKET}/${key.replace(/^private\//, '')}`, { headers: { apikey: supaHeaders.apikey, Authorization: supaHeaders.Authorization } }); if (!r.ok) return null; return { buf: Buffer.from(await r.arrayBuffer()), mime: r.headers.get('content-type') || 'application/octet-stream' }; }
    const full = path.resolve(UPLOAD_DIR, key); if (!full.startsWith(UPLOAD_DIR + path.sep) || !fs.existsSync(full)) return null;
    const ext = path.extname(full).toLowerCase(); return { buf: fs.readFileSync(full), mime: { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp', '.pdf': 'application/pdf', '.heic': 'image/heic' }[ext] || 'application/octet-stream' };
  }
  // GET /api/files/<key>?exp&sig — public route (no session), only with a valid, unexpired signature
  async function serveSignedFile(req, res, url) {
    const key = decodeURIComponent(url.pathname.replace(/^\/api\/files\//, ''));
    if (!safeKey(key) || !key.startsWith('private/') || !fileSigOk(key, url.searchParams.get('exp'), url.searchParams.get('sig'))) { res.writeHead(403, { 'Content-Type': 'text/plain' }); return res.end('This link has expired — open the document again from the portal.'); }
    const f = await readPrivateFile(key); if (!f) { res.writeHead(404); return res.end('Not found'); }
    res.writeHead(200, { 'Content-Type': f.mime, 'Cache-Control': 'private, no-store', 'Content-Disposition': `inline; filename="${key.split('/').pop().replace(/^\d+-/, '')}"` }); res.end(f.buf);
  }
  const KEY_OK = /^(private\/)?[\w-]{1,40}\/\d{6,20}-[\w.\-]{1,140}$/; // <owner>/<timestamp>-<name> — the only shape uploads ever produce
  const safeKey = (k) => (typeof k === 'string' && KEY_OK.test(k) ? k : null);
  async function deleteFile(key) {
    key = safeKey(key); if (!key) return;
    try {
      if (SUPA) await fetch(`${SUPA_URL}/storage/v1/object/${key.startsWith('private/') ? PRIVATE_BUCKET + '/' + key.replace(/^private\//, '') : 'portal-files/' + key}`, { method: 'DELETE',
        headers: { apikey: supaHeaders.apikey, Authorization: supaHeaders.Authorization } });
      else { const full = path.resolve(UPLOAD_DIR, key); if (full.startsWith(UPLOAD_DIR + path.sep)) fs.unlinkSync(full); }
    } catch (e) { /* best effort */ }
  }
  function serveUpload(res, urlPath) {
    const full = path.join(UPLOAD_DIR, path.normalize(urlPath.replace(/^\/uploads\//, '')));
    if (!full.startsWith(UPLOAD_DIR + path.sep) || full.startsWith(path.join(UPLOAD_DIR, 'private') + path.sep)) { res.writeHead(403); return res.end(); }
  ...
  const materialView = (m) => {
    ...
    return inv ? { ...m, quantity: inv.quantity, status: inv.status === 'Reserved' ? 'Reserved' : m.status,
      valueGBP: m.valueGBP || +((inv.price || 0) * inv.quantity).toFixed(2), listedPrice: inv.price, priceUnit: inv.priceUnit, retailNew: inv.retailNew || 0,
      availability: inv.archived ? 'Sold' : (inv.status === 'Pending' ? 'Available' : inv.status), fulfilment: inv.fulfilment || '',
      buyUrl: hooks.buyUrl && !inv.archived && inv.status === 'Available' && inv.quantity > 0 && inv.price > 0 ? hooks.buyUrl(inv.sku, 'buy') : '',
      reserveUrl: hooks.buyUrl && !inv.archived && inv.status === 'Available' && inv.quantity > 0 && inv.price > 0 ? hooks.buyUrl(inv.sku, 'deposit') : '' } : m;
  };
  ...
  // documents in a private bucket are handed out as 10-minute signed links — never a permanent URL
  const docOut = (d) => (d.key && d.key.startsWith('private/') ? { ...d, url: signFile(d.key) } : d);
  const projectBundle = (p) => {
    const materials = db.materials.filter((m) => m.projectId === p.id).map(materialView);
    const documents = db.documents.filter((d) => d.projectId === p.id).sort((a, b) => (b.date || '').localeCompare(a.date || '')).map(docOut);
  ...
  const csvLine = (arr) => arr.map((v) => { let s = String(v ?? ''); if (/^[=+\-@\t\r]/.test(s) && isNaN(+s)) s = `'${s}`; return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s; }).join(',');
  ...
  async function handle(req, res, url, actor, isAdmin) {
    ...
    if (m === 'POST' && url.pathname === '/api/uploads') {
      ...
      if (!kind) return json(res, 415, { error: 'Only photos (JPG, PNG, WebP, HEIC, GIF) and PDFs can be uploaded.' }), true;
      try {
        const owner = scope(b.userId);
        const f = await storeFile(owner, b.name, kind === 'image/heic' ? 'image/heic' : kind, buf, b.private === true);
    ...
    if (m === 'GET' && url.pathname === '/api/meta') {
      return json(res, 200, { stages: STAGES, materialStatuses: MATERIAL_STATUSES, requestStatuses: REQUEST_STATUSES,
        docCategories: DOC_CATEGORIES, depositPercent: +process.env.DEPOSIT_PERCENT || 50, categories: [...new Set(Object.keys(FACTORS.categories).concat(db.materials.map((x) => x.category), db.inventory.map((i) => i.category)).filter(Boolean))].sort() }), true;
    }
    ...
    if (m === 'GET' && url.pathname === '/api/documents') {
      ...
      docs = docs.map((d) => ({ ...docOut(d), projectName: d.projectId && projectById(d.projectId) ? projectById(d.projectId).name : null }))
    ...
    if (m === 'POST' && url.pathname === '/api/documents') {
      ...
      const d = { id: `DOC-${uid()}`, userId: owner, projectId: b.projectId || null, name: String(b.name).trim(),
        category: DOC_CATEGORIES.includes(b.category) ? b.category : 'Other', date: /^\d{4}-\d{2}-\d{2}$/.test(String(b.date || '')) ? String(b.date) : today(),
        url: okUrl(b.url) ? b.url : null, key: safeKey(b.key), size: num(b.size), mime: String(b.mime || '').slice(0, 60), note: String(b.note || '').slice(0, 500),
        uploadedBy: isAdmin ? 'Material Reuse Group' : actor.name, createdAt: new Date().toISOString() };
      db.documents.push(d); saveDocuments();
      return json(res, 201, { document: docOut(d) }), true;
    }
    ...
    if (m === 'POST' && url.pathname === '/api/requests') {
      ...
      const r = { id: nextId(db.requests, 'REQ-', 4), userId: owner, projectId, newProject, materials: mats,
        location: String(b.location).trim().slice(0, 300), desiredDate: /^\d{4}-\d{2}-\d{2}$/.test(String(b.desiredDate || '')) ? String(b.desiredDate) : null, notes: String(b.notes || '').slice(0, 2000),
        photos: cleanPhotos(b.photos, 10),
        status: 'Submitted', adminNote: '', history: [{ status: 'Submitted', date: today(), note: 'Request received' }],
        createdAt: new Date().toISOString() };
      if (Array.isArray(b.passportIds)) { // materials offered back for another life
        r.passportIds = b.passportIds.filter((id) => db.materials.some((x) => x.id === id && x.userId === r.userId));
        r.passportIds.forEach((id) => addHistory(db.materials.find((x) => x.id === id), 'Offered back', `Offered for reuse again by its owner (${r.id})`));
        if (r.passportIds.length) saveMaterials();
      }
    ...
    if (parts[1] === 'admin') {
      ...
      if (parts[2] === 'requests' && parts[3]) {
        ...
        if (m === 'POST' && parts[4] === 'passports') {
          ...
          const created = r.materials.map((x) => {
            ...
            mat.ref = mat.id; estimateMaterial(mat); addHistory(mat, 'Recovered', `Recovered from ${r.location}${r.newProject ? ` — ${r.newProject.name}` : ''}`); db.materials.push(mat); return mat;
          });
      ...
      if (m === 'POST' && url.pathname === '/api/admin/materials') {
        ...
        addHistory(mat, mat.status, `Passport created${mat.sourceBuilding ? ` — ${mat.sourceBuilding}` : ''}`, mat.dateRecovered);
        db.materials.push(mat); saveMaterials(); applyMaterialStats(mat);
        if (!mat.userId && hooks.stock) hooks.stock(mat, b.stock, actor);
        saveMaterials();
      ...
      if (parts[2] === 'materials' && parts[3]) {
        ...
        if (m === 'PATCH') {
          ...
          if (mat.status !== prevStatus) addHistory(mat, mat.status, mat.status === 'Rehomed' ? `Rehomed${mat.reuseDestination ? ` — ${mat.reuseDestination}` : ''}` : mat.status === 'Recycled' ? 'Sent for recycling' : mat.status === 'In storage' ? 'Arrived in the Material Reuse Group warehouse' : `Status changed to ${mat.status}`, mat.status === 'Rehomed' ? mat.dateRehomed : undefined);
  ...
  function applyMaterialFields(mat, b) {
    ...
    if (Array.isArray(b.photos)) mat.photos = cleanPhotos(b.photos, 12);
  ...
  return { hooks, fire, addHistory, storeFile, estimateMaterial, factorFor, addEvent, saveMaterialsDirect: saveMaterials,
    STAGES, MATERIAL_STATUSES, REQUEST_STATUSES, DOC_CATEGORIES, migrate, handle, serveUpload, serveSignedFile, signFile,
