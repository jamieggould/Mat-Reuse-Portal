...
module.exports = function install(ctx) {
  ...
  const cleanPhotos = (arr, max = 12) => (Array.isArray(arr) ? arr : []).filter((p) => p && okUrl(p.url)).slice(0, max).map((p) => ({ url: p.url, key: p.key || null, name: String(p.name || 'photo').slice(0, 120) }));
  ...
  async function handle(req, res, url, actor, isAdmin) {
    ...
    if (m === 'POST' && url.pathname === '/api/uploads') {
      ...
      try {
        ...
        const f = await storeFile(owner, b.name, kind === 'image/heic' ? 'image/heic' : kind, buf);
    ...
    if (m === 'POST' && url.pathname === '/api/documents') {
      ...
      const d = { id: `DOC-${uid()}`, userId: owner, projectId: b.projectId || null, name: String(b.name).trim(),
        ...
        url: okUrl(b.url) ? b.url : null, key: b.key || null, size: num(b.size), mime: String(b.mime || '').slice(0, 60), note: String(b.note || '').slice(0, 500),
    ...
    if (m === 'POST' && url.pathname === '/api/requests') {
      ...
      if (Array.isArray(b.passportIds)) { // materials offered back for another life
        r.passportIds = b.passportIds.filter((id) => db.materials.some((x) => x.id === id));
