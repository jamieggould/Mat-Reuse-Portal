...
module.exports = function install(ctx) {
  ...
  const materialView = (m) => {
    ...
    return inv ? { ...m, quantity: inv.quantity, status: inv.status === 'Reserved' ? 'Reserved' : m.status,
      valueGBP: m.valueGBP || +((inv.price || 0) * inv.quantity).toFixed(2), listedPrice: inv.price, priceUnit: inv.priceUnit } : m;
  };
      ...
      if (parts[2] === 'requests' && parts[3]) {
        ...
        if (m === 'POST' && parts[4] === 'passports') {
          ...
          const created = r.materials.map((x) => {
            ...
            mat.ref = mat.id; db.materials.push(mat); return mat;
          });
  ...
  return { STAGES, MATERIAL_STATUSES, REQUEST_STATUSES, DOC_CATEGORIES, migrate, handle, serveUpload,
