...
module.exports = function install(ctx) {
  ...
  async function recordOrderSession(session) {
    ...
    const r = recordOrder({ inv, quantity, user, deposit: md.mode === 'deposit', unitPrice: num(md.unitPrice, inv.price), paid: +((session.amount_total || 0) / 100).toFixed(2),
      creditApplied: Math.min(num(md.creditApplied), creditInfo(user).remaining), sessionId: session.id, customerId: session.customer || null });
    if (num(md.creditApplied) > num(r.order.creditApplied)) { // credit was spent elsewhere while this session was open — flag the shortfall for the team
      r.order.balanceDueGBP = +(num(r.order.balanceDueGBP) + num(md.creditApplied) - num(r.order.creditApplied)).toFixed(2);
      r.order.note = `${r.order.note || ''} · Credit shortfall ${gbp(num(md.creditApplied) - num(r.order.creditApplied))} — balance to settle`.replace(/^ · /, '');
      audit(null, 'credit shortfall', user.id, `${r.order.id}: ${gbp(num(md.creditApplied) - num(r.order.creditApplied))}`);
    }
  ...
  async function completeSession(sessionId) {
    ...
    if (session.status !== 'complete') return null; // unpaid / expired / still open — never act on it
  ...
  async function applySignupSession(session) {
    ...
    if (!session.subscription || !md.tierId || !tierById(md.tierId)) return null; // no subscription = nothing was paid
  ...
  const page = (title, body) => `<!DOCTYPE html><html lang="en-GB"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} — Material Reuse Hub</title><link rel="icon" href="/assets/favicon.png" type="image/png">
...
<body><div class="card"><div class="head"><img src="/assets/mrg-logo-group-sm.png" alt="Material Reuse Group"></div><div class="body">${body}</div><div class="foot">Material Reuse Group · material-reuse.co.uk · 01932 867989</div></div></body></html>`;
  ...
  function signupCompletePage(r) {
    return page('Account created', r && r.user ? `<h1>Welcome to the Material Reuse Hub, ${esc(r.user.name.split(' ')[0])}</h1><p>Your <b>${esc((tierById(r.user.tier) || {}).name || '')}</b> is live and paid. Sign in with the email and password you chose.</p><a class="btn" href="/">Sign in</a>`
  ...
  return { ENV, listing, publicListings, publicTiers, onSale, buyUrl, upsertStock, createCheckout, memberCheckout, creditInfo, isCommunity, evidenceOutstanding, startSignup, applySignupSession, signupCompletePage, completeSession, pollSessions, recordOrderSession,
    membershipView, changeMembership, cancelPending, billingPortalUrl, syncSubscriptions, ensurePrices,
