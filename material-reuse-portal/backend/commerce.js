...
module.exports = function install(ctx) {
  ...
  function listing(i) {
    ...
    return { sku: i.sku, passportId: m ? m.id : i.sku, title: i.title, category: i.category, itemType: i.itemType || '', quantity: i.quantity, unit: i.unit || 'items',
      price: i.price, priceUnit: i.priceUnit || 'each', carbonSavedKgPerUnit: i.carbonSavedKgPerUnit || 0, fulfilment: i.fulfilment || 'Collection Only',
  ...
  async function recordOrderSession(session) {
    ...
    const { user, created } = await ensureMember(email, cd.name, cd.phone);
    if (!user) return null;
    const deposit = md.mode === 'deposit';
    const unitPrice = num(md.unitPrice, inv.price);
    ...
    const order = { id: `ORD-${new Date().getFullYear()}-${nextOrderSeq()}`, userId: user.id, placed: today(), status: deposit ? 'Reserved' : 'Awaiting collection',
      fulfilment: inv.fulfilment === 'Available For Delivery' ? 'Delivery available — arrange with the team' : 'Collection — Material Reuse Group warehouse',
      slot: 'Slot to be confirmed — we’ll be in touch', items: [{ sku: inv.sku, title: inv.title, qty, price: unitPrice }], total,
      carbonSavedKg: +((inv.carbonSavedKgPerUnit || 0) * qty).toFixed(1), stripeSessionId: session.id, stripeCustomerId: session.customer || null,
      note: deposit ? `Reserved online — ${ENV.DEPOSIT_PERCENT}% deposit paid, balance payable before collection` : 'Purchased online — paid in full', source: 'marketplace' };
    if (deposit) { order.depositGBP = paid; order.balanceDueGBP = +(total - paid).toFixed(2); }
    db.orders.unshift(order);
    inv.quantity = Math.max(0, (inv.quantity || 0) - qty);
    if (inv.quantity === 0) inv.status = deposit ? 'Reserved' : 'Sold';
    saveInventory(); saveOrders();
    if (mat) { features.addHistory(mat, deposit ? 'Reserved' : 'Sold', `${qty} ${inv.unit || 'items'} ${deposit ? 'reserved' : 'bought'} by a member (${order.id})`); if (inv.quantity === 0 && !deposit) mat.status = 'Rehomed'; else if (inv.quantity === 0) mat.status = 'Reserved'; features.saveMaterialsDirect(); }
    audit(null, deposit ? 'reservation' : 'purchase', order.id, `${qty} × ${inv.title} · ${gbp(paid)} · ${user.email}${created ? ' (account created)' : ''}`);
    mail.orderConfirmed(order, user, inv);
    sendSms(user, `Thanks — order ${order.id} (${qty} × ${inv.title}) is confirmed. We'll text you when your collection slot is booked.`);
  ...
  async function pollSessions() {
    ...
    for (let page = 0; page < 5; page++) {
      ...
      for (const s of j.data || []) {
        ...
        try { if (s.mode === 'subscription') await applyMembershipSession(s); else await recordOrderSession(s); } catch (e) { console.error(`  session ${s.id}:`, e.message); }
      }
  ...
  /* =================================================================
     ...
     ================================================================= */
  const paidTiers = () => db.tiers.filter((t) => t.price > 0);
  ...
  function membershipView(u) {
    ...
    return { tier: u.tier, tierName: t ? t.name : u.tier, renews: u.tierRenews || null, pendingTier: u.pendingTier || null, pendingAt: u.pendingAt || null,
      communityRequest: u.communityRequest || null, subscription: !!u.stripeSubscriptionId, paymentIssue: !!u.paymentIssue, stripe: stripeOn };
  }
  ...
  async function changeMembership(u, tierId, opts = {}) {
    ...
    if (tierId === 'community') {
      if (!opts.organisation) throw new Error('Tell us the name of your community group or organisation.');
      u.communityRequest = { organisation: String(opts.organisation).slice(0, 120), at: new Date().toISOString() }; saveUsers();
      mail.communityRequest(u, u.communityRequest.organisation); audit(u, 'requested community membership', u.id, u.communityRequest.organisation);
      return { ok: true, message: 'Request sent — Kallie will review it and you’ll get an email. Your current plan carries on meanwhile.' };
    }
...
<body><div class="card"><div class="head"><img src="/assets/mrg-logo-group-sm.png" alt="Material Reuse Group"></div><div class="body">${body}</div><div class="foot">Material Reuse Group · material-reuse.co.uk · 01932 867989</div></div></body></html>`;
  ...
  return { ENV, listing, publicListings, onSale, buyUrl, upsertStock, createCheckout, completeSession, pollSessions, recordOrderSession,
