...
module.exports = function install(ctx) {
  ...
  async function walkInSale(mat, { email, name, phone, qty }, actor) {
    const { user, created } = await integrations.ensureMember(email, name, phone);
    if (!user) throw new Error('Could not create an account for that email.');
    ...
    const order = { id: `ORD-${new Date().getFullYear()}-${ctx.nextOrderSeq()}`, userId: user.id, placed: new Date().toISOString().slice(0, 10), status: 'Collected',
    ...
    if (inv) { inv.quantity = Math.max(0, (inv.quantity || 0) - q); if (inv.quantity === 0) inv.status = 'Sold'; saveInventory(); }
    else { mat.quantity = Math.max(0, num(mat.quantity) - q); if (mat.quantity === 0) mat.status = 'Rehomed'; }
    features.applyOrderStats(order); saveOrders(); features.saveMaterialsDirect();
    if (inv && inv.airtableId) {
      ...
      const rowId = await atCreate({ 'Order ID': order.id, Item: [inv.airtableId], 'Item Name': inv.title, Quantity: q, 'Unit Price': price, 'Order Total': order.total, 'Amount Paid': order.total, 'Balance Due': 0,
        'Order Type': 'Purchase', 'Buyer Name': user.name, 'Buyer Email': user.email, 'Buyer Phone': user.phone || '', 'Order Date': new Date().toISOString(), 'Collection Status': 'Collected', Source: 'Portal', Notes: 'Walk-in sale — paid in person' });
      if (rowId) { order.airtableOrderId = rowId; order.airtableId = inv.airtableId; saveOrders(); }
    }
    audit(actor, 'walk-in sale', order.id, `${q} × ${mat.name} → ${user.email}${created ? ' (account created)' : ''}`);
  ...
  async function runNow() { await flushAlerts(); await checkBalanceInvoices(); await syncStripeInvoices(); await staleSweep(); }
  function start() {
  ...
  return { ENV, audit, auditRequest, sendSms, sendBalanceInvoice, checkBalanceInvoices, syncStripeInvoices, walkInSale, onSlotConfirmed, onNewListings, flushAlerts, staleSweep, releaseHold, sendDigest, digestData, publicImpact, badgeFor, badgeSvg, status, start, runNow, slotConfirmed };
};
