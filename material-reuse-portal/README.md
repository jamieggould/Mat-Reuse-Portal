...
### File uploads: Supabase Storage buckets: `portal-files` (public — photos) and `portal-private` (private — member documents, created automatically on first document upload; served via signed links). Storage bucket (one-off)
...
### Marketplace — memberships, credit, community rules
...
- **Credit** is applied automatically at checkout (Buy or Reserve), any item, and resets each membership year. If credit covers the whole order it's claimed without Stripe ("Buy with credit"). Quantity is chosen in the marketplace before checkout.
- Domestic tiers see only what's relevant: Dashboard (credit, collections, orders), Marketplace, Orders, Membership, Settings. Passports library, Documents, Impact and the badge are for Community/Corporate.
...
### Airtable mirror + AI listing help

- **Portal → Airtable**: every listing created or changed in the portal (price, quantity, availability, photos, sales) is mirrored to Table 1 within a few seconds, one API call per change (well inside the free plan). Needs `AIRTABLE_TOKEN` on Render; set `AIRTABLE_PUSH=false` to stop it. The old Airtable → portal pull stays off unless `AIRTABLE_SYNC=true`.
- **Suggest with AI** on the admin passport form: recommended price (15–20% above what comparable items sell for in the warehouse, never above 70% of new), embodied carbon saved per item, and what it would cost new. Shown as a suggestion; Kallie types the numbers. Needs `ANTHROPIC_API_KEY`.
- **Cost new** appears on listing cards, the passport and the public marketplace as "≈ £X new · save Y%".
