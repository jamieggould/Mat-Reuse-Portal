...
3. Render → Environment → `RESEND_API_KEY` = the key. Optional: `MAIL_FROM` (default `Material Reuse Group <portal@material-reuse.co.uk>`), `ADMIN_EMAIL` (default kallie@material-reuse.co.uk), `PORTAL_URL` (default the Render URL — change when you add a custom domain).
