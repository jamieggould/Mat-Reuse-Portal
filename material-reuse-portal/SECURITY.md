...
## Second review (21 Sep 2026) — what was found and fixed

| Risk | What an attacker could do | Now |
|---|---|---|
| **Unpaid sign-up accepted** (critical) | Start a paid sign-up, close Stripe without paying, then open the `/signup-complete` link — the account was created on the paid tier with its full credit | Only sessions Stripe marks `complete` with a real subscription are honoured |
| **Storage key traversal** (critical) | A member could attach `../server.js` as a document's storage key and delete it (or hit Supabase's REST API with the service key) | Keys must match the exact shape uploads produce; local deletes stay inside the uploads folder |
| **Credit double-spend** (high) | Open several part-credit checkouts, pay them all — each re-applied the same credit | Credit is re-checked when each session is recorded; any shortfall becomes a balance due and is audited |
| **Rate-limit bypass** (high) | Send your own `X-Forwarded-For` header and get a fresh limit bucket every request | The last hop (what Render itself saw) is used |
| **2FA brute force** (high) | Wrong authenticator codes reset the lockout counter | Counter only clears after the code is right |
| **Password reset skipped 2FA** (high) | A reset link signed an admin straight in for 30 days | Reset only changes the password; 2FA accounts then sign in normally, admin sessions stay 12 h |
| Session tokens in the database (medium) | A copy of the database or a backup contained usable bearer tokens | Sessions are stored as SHA-256 hashes (everyone signs in once after this deploy) |
| Email HTML injection (medium) | Member-typed dates/names went into admin emails unescaped | Escaped |
| Checkout error page (medium) | Upstream error text reflected unescaped | Escaped; a thrown error after headers can no longer hang a request |
| Members altering others' passports (medium) | "Offer back" could append history to any passport | Only your own passports |
| **Member documents on public URLs** (medium) | Uploaded audits/invoices sat at unguessable but public storage URLs | Documents now go to a private bucket (`portal-private`, created automatically) and open through signed links that expire after an hour; photos stay public as before |
| Various (low) | Profile fields accepting objects, notification key injection, list quantities, admin duplicate emails, CSV formula injection, 2FA secret overwrite, `null` JSON bodies, QR relay for other hosts | All validated |

Run `node tests/e2e.js` for the full end-to-end suite (128 checks, mock services, throwaway copy).
